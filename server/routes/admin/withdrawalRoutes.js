// server/routes/admin/withdrawalRoutes.js
...
router.get("/", ...guard, async (req, res) => {
  ...
  if (q?.trim()) {
    conditions.push(
      `(v.store_name ILIKE $${idx} OR wr.account_number ILIKE $${idx} OR wr.tx_ref ILIKE $${idx})`
    );
    values.push(`%${q.trim()}%`);
    idx++;
  }
...
router.post("/:id/approve", ...guard, async (req, res) => {
  ...
  try {
    ...
    try {
      const { initiateTransfer: transfer } = await import(
        "../../utils/flutterwave.js"
      );

      const flwResult = await transfer({
      ...
      // FLW failed — mark as failed, restore balance

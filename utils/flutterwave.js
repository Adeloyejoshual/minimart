...
const flw = () => {
  ...
  instance.interceptors.response.use(
    ...
    (err) => {
      ...
      console.error("[FLW]", { status: err.response?.status, message: msg });
...
export const getBankCode = (bankName) =>
  bankName ? (COMMERCIAL_BANKS[bankName.toLowerCase().trim()] ?? null) : null;

export const getSupportedBanks = () => {
  const seen = new Set();
  const list = [];
  Object.values(COMMERCIAL_BANKS).forEach((b) => {
    if (!seen.has(b.code)) { seen.add(b.code); list.push(b); }
  });
...
export const verifyAccountName = async (accountNumber, bankName) => {
  ...
  if (!bank) return { valid: false, message: `"${bankName}" is not a supported bank.` };
  ...
  try {
    ...
    if (!resolved) return { valid: false, message: "Could not verify account." };
    return {
      valid:          true,
...
// ── Fee schedule ──────────────────────────────────────────────────────────────
//  First 3 withdrawals today → FREE
...
export const feeScheduleLabel = (withdrawalsToday) => {
  ...
  return free > 0
    ...
    : "Fees: ₦50 / ₦100 / ₦150 / ₦200 (by amount)";
};

/**
 * DB-aware fee helper — fetches today's withdrawal count then computes fee.
 * Pass a pool or connected client.
 */
...
export const initiateTransfer = async ({
  vendorId, amount, fee, netAmount,
  bankName, accountNumber, txRef,
}) => {
  const bank = getBankCode(bankName);
  ...
  const { data } = await flw().post("/transfers", {
    ...
    debit_currency: "NGN",
    meta: { vendor_id: String(vendorId), tx_ref: txRef, gross: amount, fee },
  });

  if (data.status !== "success") throw new Error(data.message ?? "Transfer failed");

  console.log("[FLW] transfer initiated:", {
    flw_id: data.data.id, reference: data.data.reference,
    amount: data.data.amount, bank: bank.name,
  });

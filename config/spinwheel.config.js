...
import {
  WHEEL_SEGMENTS,
  EARN_TASKS,   // ← the only correct name; EARN_TASKS_DEF never existed
  TASK_MAP,
  SPIN_CONSTANTS,
...
/*
 * Pre-computed task stats that are constant for the lifetime of
 * the process. Avoids repeating .reduce() on every /tasks request.
 */
const TASK_COUNT     = EARN_TASKS.length;
const MAX_TASK_SPINS = EARN_TASKS.reduce((s, t) => s + t.spins_reward, 0);
...
router.get("/tasks/all", authenticate, async (req, res) => {
  ...
  try {
    ...
    /*
     * Use EARN_TASKS (the correct export name) — not EARN_TASKS_DEF.
     */
    const tasks = EARN_TASKS.map((t) => ({
      ...t,
      stats: countMap[t.id] ?? {

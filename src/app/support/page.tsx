...
const SUPPORT_OPTIONS = [
  ...
  },
  {
    ...
    variant: 'option-secondary',
  },
  {
    title: 'Report a Problem',
    description: 'Report scams, fraud, or violations',
    icon: IconFlag,
    href: '/support/report',
    variant: 'option-danger',
  },
  {
    title: 'Open a Dispute',
    description: 'Resolve buyer-seller issues',
    icon: IconScale,
    href: '/support/disputes',
    variant: 'option-warning',
  },
  {
    title: 'File an Appeal',
    description: 'Appeal account or listing decisions',
    icon: IconMegaphone,
    href: '/support/appeals',
    variant: 'option-success',
  },
  {
    title: 'Leave Feedback',
    description: 'Rate support and suggest features',
    icon: IconStar,
    href: '/support/feedback',
    variant: 'option-accent',
  },
];
    ...
    <div className="support-hub-page">
      <div className="support-hub-container">
        ...
        <div className="support-hub-grid">
          {SUPPORT_OPTIONS.map(
            ({ title, description, icon: Icon, href, variant }) => (
              <Link
                key={href}
                href={href}
                className={`support-hub-card ${variant}`}
              >
                ...
                <IconArrowRight
                  size={16}
                  className="support-hub-card-arrow"
                />
              </Link>
            )
          )}
        ...
        <div className="support-hub-response-section">
          <h3 className="support-hub-response-title">
          ...
          <div className="support-hub-response-grid">
            {RESPONSE_TIMES.map(({ label, time, variant }) => (
              <div
                key={label}
                className={`support-hub-response-card ${variant}`}
              >

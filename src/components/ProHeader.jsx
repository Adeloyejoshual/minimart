...
export default function ProductDetail() {
  ...
  /* ── Re-sync fav state if userId changes (account switch) ── */
  useEffect(() => {
    if (!product?.id) return;
    setFav(!!loadFavs(userId)[product.id]);
  }, [userId, product?.id]);
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      // Load favorites for THIS user only — won't bleed across accounts
      setFav(!!loadFavs(userId)[data.id]);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);

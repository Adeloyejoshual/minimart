...
const StatusBadgeInternal = ({ status }) => {
...
export const StatusBadge = StatusBadgeInternal;
  ...
  <>
    ...
    <aside className={`sd-sidebar ${sidebarOpen ? "open" : ""}`}>
      <div className="sd-sidebar-header">
        {vendor?.store_logo ? (
          <img src={vendor.store_logo} alt="" className="sd-store-logo" />
        ) : (
        ...
        <div className="sd-store-info">
          ...
          <StatusBadgeInternal status={vendor?.status} />
        </div>
        <button
...
const STAT_CONFIG = [
  { key: "total_revenue",  label: "Total Revenue",   icon: "💰", color: "#10b981", bg: "#ecfdf5", format: (v) => formatNGN(v),                        changeKey: "revenue_change" },
  { key: "total_orders",   label: "Total Orders",    icon: "📦", color: "#6366f1", bg: "#eef2ff", format: (v) => Number(v ?? 0).toLocaleString(),      changeKey: "orders_change"  },
  { key: "total_products", label: "Products",        icon: "🏷️", color: "#8b5cf6", bg: "#f5f3ff", format: (v) => Number(v ?? 0).toLocaleString()                                  },
  { key: "pending_orders", label: "Pending Orders",  icon: "⏳", color: "#f59e0b", bg: "#fffbeb", format: (v) => Number(v ?? 0).toLocaleString()                                  },
  { key: "total_customers",label: "Customers",       icon: "👥", color: "#3b82f6", bg: "#eff6ff", format: (v) => Number(v ?? 0).toLocaleString()                                  },
  { key: "avg_order_value",label: "Avg Order",       icon: "📊", color: "#ec4899", bg: "#fdf2f8", format: (v) => formatNGN(v)                                                     },
];
...
export const Overview = ({ dash }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
    ...
    <div className="sd-quick-actions">
      {QUICK_ACTIONS.map((a) =>
        a.href ? (
          <a key={a.label} href={a.href} className="sd-quick-btn"
            style={{ background: a.bg, color: a.color }}>
            <span className="sd-quick-icon">{a.icon}</span>
            <span className="sd-quick-label">{a.label}</span>
          </a>
        ) : (
          <button key={a.label} className="sd-quick-btn"
            style={{ background: a.bg, color: a.color, border: "none", cursor: "pointer" }}
    ...
    <div className="sd-stats-section">
      <div className="sd-time-selector">
        {TIME_RANGES.map((r) => (
          <button key={r.value}
            className={`sd-time-btn ${dash.timeRange === r.value ? "active" : ""}`}
            onClick={() => dash.setTimeRange(r.value)}>
            {r.label}
          </button>
        ))}
      </div>
      <div className="sd-stats-grid">
        {STAT_CONFIG.map((cfg) => {
          const value  = dash.stats?.[cfg.key];
          const change = cfg.changeKey ? dash.stats?.[cfg.changeKey] : null;
          return (
        ...
        <div className="sd-card">
          ...
          <div className="sd-chart">
            <div className="sd-chart-bars">
              {(() => {
                const maxVal = Math.max(...dash.revenueChart.map((d) => d.revenue), 1);
                return dash.revenueChart.map((d, i) => (
                  <div key={i} className="sd-chart-col">
                    <div className="sd-chart-tooltip">{formatNGN(d.revenue)}</div>
                    <div className="sd-chart-bar"
            ...
            {dash.notifications.slice(0, 5).map((n) => (
              <div key={n.id}
                ...
                <div className="sd-notif-content">
                  ...
                  <span className="sd-notif-time">{formatTimeAgo(n.created_at)}</span>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="sd-card">
      <div className="sd-card-header">
        ...
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", color: "#6366f1", fontWeight: 600, fontSize: "0.85rem" }}>
          View All →
        </button>
      </div>
          ...
          <table className="sd-table">
            ...
            <tbody>
              {dash.recentOrders.slice(0, 5).map((o) => {
                const st = ORDER_STATUS_COLORS[o.status] ?? ORDER_STATUS_COLORS.pending;
                return (
                  <tr key={o.id}>
                    <td className="sd-order-id">#{o.id?.slice(-8).toUpperCase()}</td>
                    <td>{o.customer_name ?? "—"}</td>
                    <td className="sd-order-total">{formatNGN(o.total)}</td>
                    <td>
                      <span style={{ padding: "0.2rem 0.6rem", borderRadius: "100px", fontSize: "0.72rem", fontWeight: 700, color: st.color, background: st.bg }}>
                        {o.status}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="sd-order-tabs">
        {ORDER_TABS.map((tab) => (
          <button key={tab.value}
            ...
            style={orderTab === tab.value ? { borderColor: tab.color, color: tab.color } : {}}
          ...
          <table className="sd-table">
            ...
            <tbody>
              {orders.map((order) => {
                ...
                const st = ORDER_STATUS_COLORS[order.status] ?? ORDER_STATUS_COLORS.pending;
                return (
                  <tr key={order.id}>
                    <td className="sd-order-id">#{order.id?.slice(-8).toUpperCase()}</td>
                    <td>{order.customer_name ?? "—"}</td>
                    <td style={{ textAlign: "center" }}>{order.item_count ?? 0}</td>
                    <td className="sd-order-total">{formatNGN(order.total)}</td>
                    ...
                    <td>
                      {action ? (
                        <button className="sd-action-btn"
                          ...
                          {updating === order.id ? "..." : `${action.icon} ${action.label}`}
                        </button>
                      ) : (
...
export const TopProducts = ({ products }) => (
  <div className="sd-card">
    <div className="sd-card-header">
      ...
      <a href="/minimart/add" style={{ color: "#6366f1", fontWeight: 600, fontSize: "0.85rem", textDecoration: "none" }}>
        ➕ Add Product
      </a>
    </div>
    ...
    {!products?.length ? (
      <div className="sd-empty">
        <p>No products yet</p>
        <a href="/minimart/add" className="sd-empty-cta">➕ Add Your First Product</a>
      </div>
    ) : (
        ...
        {products.map((p, i) => (
          <div key={p.id ?? i} className="sd-product-row">
            ...
            <div style={{ textAlign: "right" }}>
              <div className="sd-product-revenue">{formatNGN(p.revenue)}</div>
              <div style={{ fontSize: "0.72rem", color: "#9ca3af" }}>revenue</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "1rem" }}>
        {[
          ...
          <div key={item.label} style={{ background: "white", borderRadius: "12px", padding: "1.25rem", display: "flex", alignItems: "center", gap: "1rem", border: "1px solid #f3f4f6", borderLeft: `4px solid ${item.color}` }}>
            <span style={{ fontSize: "1.5rem" }}>{item.icon}</span>
            <div>
              <p style={{ fontWeight: 800, fontSize: "1.1rem", color: item.color, margin: 0 }}>{item.value}</p>
              <p style={{ color: "#9ca3af", fontSize: "0.8rem", margin: 0 }}>{item.label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="sd-card">
        ...
        {!data?.length ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: "1.5rem", paddingTop: "1rem", borderTop: "1px solid #f3f4f6", flexWrap: "wrap" }}>
              {[
                { label: "Total",   value: formatNGN(data.reduce((s, d) => s + Number(d.revenue), 0)) },
                { label: "Peak",    value: formatNGN(Math.max(...data.map((d) => d.revenue)))         },
                { label: "Average", value: formatNGN(data.reduce((s, d) => s + Number(d.revenue), 0) / data.length) },
              ].map(({ label, value }) => (
                <div key={label} style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                  ...
                  <span style={{ fontSize: "1rem",    color: "#1f2937", fontWeight: 800 }}>{value}</span>
                </div>
              ))}
            </div>
          </>
...
export const Payouts = ({ vendor }) => {
    ...
    const load = async () => {
      try {
        const [b, t, w] = await Promise.all([
          axios.get("/api/seller-wallet/balance",             { headers: authHead() }),
          axios.get("/api/seller-wallet/transactions?limit=10", { headers: authHead() }),
          axios.get("/api/seller-wallet/withdrawals?limit=5",  { headers: authHead() }),
        ]);
        setWallet(b.data);
  ...
  const handleWithdraw = async () => {
    ...
    if (amount > available) {
      setMsg({ type: "error", text: `Insufficient. Available: ${formatNGN(available)}` });
      return;
    }
    setWithdrawing(true);
    setMsg({ type: "", text: "" });
    try {
      const { data } = await axios.post("/api/seller-wallet/withdraw", { amount }, { headers: authHead() });
      setMsg({ type: "success", text: `✅ ${data.message}` });
      setWithdrawAmt("");
      const r = await axios.get("/api/seller-wallet/balance", { headers: authHead() });
      setWallet(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "1rem" }}>
        {[
          ...
          <div key={card.label} style={{
            background:   card.primary ? `linear-gradient(135deg,${card.color},${card.color}bb)` : "white",
            borderRadius: "16px", padding: "1.25rem",
            border:       card.primary ? "none" : "1px solid #f3f4f6",
            boxShadow:    card.primary ? `0 4px 20px ${card.color}30` : "0 1px 3px rgba(0,0,0,0.04)",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem" }}>
              <span style={{ fontSize: "1.25rem" }}>{card.icon}</span>
              <span style={{ fontSize: "0.8rem", fontWeight: 500, color: card.primary ? "rgba(255,255,255,0.85)" : "#9ca3af" }}>
                {card.label}
              </span>
            </div>
            <div style={{ fontSize: "1.5rem", fontWeight: 800, color: card.primary ? "white" : card.color }}>
      ...
      {va ? (
        <div className="sd-card">
          ...
          <p style={{ color: "#6b7280", fontSize: "0.875rem", marginBottom: "1rem" }}>
            Share this account number with buyers
          </p>
          <div style={{ background: "#f8fafc", borderRadius: "12px", padding: "1rem", border: "1px solid #e5e7eb" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.5rem 0", borderBottom: "1px solid #f3f4f6" }}>
              <span style={{ color: "#6b7280", fontSize: "0.85rem", fontWeight: 500 }}>Account Number</span>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <span style={{ fontWeight: 800, fontSize: "1.1rem", fontFamily: "monospace", letterSpacing: "0.08em", color: "#6366f1" }}>
                  {va.account_number}
                </span>
                <button
                  onClick={() => { navigator.clipboard.writeText(va.account_number); alert("Copied!"); }}
                  style={{ padding: "0.2rem 0.65rem", background: "#eef2ff", color: "#6366f1", border: "none", borderRadius: "6px", fontWeight: 600, fontSize: "0.75rem", cursor: "pointer" }}>
                  📋 Copy
                </button>
              </div>
            </div>
            {[
              { label: "Account Name", value: va.account_name },
          ...
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "0.75rem 1rem", color: "#1e40af", fontSize: "0.82rem", marginTop: "1rem" }}>
        ...
        <div className="sd-card">
          <div style={{ textAlign: "center", padding: "2rem", color: "#9ca3af" }}>
            ...
            <p style={{ fontSize: "0.875rem" }}>Created automatically when your store is activated.</p>
          </div>
        </div>
      )}
      ...
      <div className="sd-card">
        ...
        {/* Bank info */}
        <div style={{ background: "#f8fafc", borderRadius: "12px", padding: "1rem", border: "1px solid #e5e7eb", marginBottom: "1rem" }}>
          {[
        ...
        <div style={{ position: "relative" }}>
          <span style={{ position: "absolute", left: "1rem", top: "50%", transform: "translateY(-50%)", fontWeight: 700, color: "#374151", fontSize: "1rem" }}>₦</span>
          <input
            type="number"
            placeholder="Enter amount (min ₦500)"
            value={withdrawAmt}
            onChange={(e) => setWithdrawAmt(e.target.value)}
            style={{ width: "100%", padding: "0.875rem 1rem 0.875rem 2rem", border: "2px solid #e5e7eb", borderRadius: "12px", fontSize: "1.1rem", fontWeight: 600, outline: "none", boxSizing: "border-box" }}
          />
        </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.75rem", flexWrap: "wrap" }}>
          {[1000, 5000, 10000, 50000].map((amt) => (
            <button key={amt} onClick={() => setWithdrawAmt(String(amt))} style={{
              padding: "0.4rem 0.875rem", borderRadius: "100px", border: "1px solid", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600,
          ...
          <button
            ...
            style={{ padding: "0.4rem 0.875rem", borderRadius: "100px", border: "1px solid #6366f1", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, background: "white", color: "#6366f1" }}>
            All
          </button>
        </div>

        {msg.text && (
          <div style={{ padding: "0.75rem 1rem", borderRadius: "10px", fontSize: "0.875rem", marginTop: "1rem", background: msg.type === "error" ? "#fef2f2" : "#ecfdf5", color: msg.type === "error" ? "#991b1b" : "#065f46", border: `1px solid ${msg.type === "error" ? "#fecaca" : "#a7f3d0"}` }}>
            {msg.text}
          </div>
        )}
        ...
        <button
          ...
          style={{ width: "100%", padding: "1rem", marginTop: "1rem", background: "linear-gradient(135deg,#10b981,#059669)", color: "white", border: "none", borderRadius: "14px", fontWeight: 700, fontSize: "1rem", cursor: "pointer", opacity: withdrawing || !withdrawAmt ? 0.6 : 1 }}>
      ...
      {transactions.length > 0 && (
        <div className="sd-card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {transactions.map((tx) => (
              <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem", background: "#f8fafc", borderRadius: "10px" }}>
                <span style={{ fontSize: "1.1rem", width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "8px", background: tx.type === "credit" ? "#ecfdf5" : "#fef2f2" }}>
                  {tx.type === "credit" ? "📥" : "📤"}
                </span>
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 600, color: "#1f2937", margin: 0, fontSize: "0.875rem" }}>{tx.narration ?? tx.type}</p>
                  <p style={{ color: "#9ca3af", margin: 0, fontSize: "0.75rem" }}>{new Date(tx.created_at).toLocaleDateString("en-NG")}</p>
                </div>
                <span style={{ fontWeight: 800, fontSize: "0.9rem", color: tx.type === "credit" ? "#10b981" : "#ef4444" }}>
      ...
      {withdrawals.length > 0 && (
        <div className="sd-card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {withdrawals.map((wd) => {
              const wdSt = { success: { color: "#10b981", bg: "#ecfdf5" }, pending: { color: "#f59e0b", bg: "#fffbeb" }, failed: { color: "#ef4444", bg: "#fef2f2" } }[wd.status] ?? { color: "#f59e0b", bg: "#fffbeb" };
              return (
                <div key={wd.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem", background: "#f8fafc", borderRadius: "10px" }}>
                  <span style={{ fontSize: "1.1rem", width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "8px", background: "#fef2f2" }}>💸</span>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontWeight: 600, color: "#1f2937", margin: 0, fontSize: "0.875rem" }}>To {wd.bank_name}</p>
                    <p style={{ color: "#9ca3af", margin: 0, fontSize: "0.75rem" }}>{new Date(wd.created_at).toLocaleDateString("en-NG")}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontWeight: 800, color: "#ef4444", margin: 0, fontSize: "0.9rem" }}>-{formatNGN(wd.amount)}</p>
                    <span style={{ fontSize: "0.7rem", fontWeight: 700, color: wdSt.color, background: wdSt.bg, padding: "0.1rem 0.45rem", borderRadius: "100px" }}>{wd.status}</span>
                  </div>
                </div>
              );
            })}
...
export const Settings = ({ vendor }) => {
  const rows = [
    ...
    { label: "Status",        value: <StatusBadgeInternal status={vendor?.status} /> },
    ...
    { label: "Member Since",  value: vendor?.created_at
        ? new Date(vendor.created_at).toLocaleDateString("en-NG", { year: "numeric", month: "long", day: "numeric" })
        : "—"
    },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div className="sd-card">
        <div className="sd-card-header">
          ...
          <a href="/become-seller" style={{ padding: "0.5rem 1rem", background: "#eef2ff", color: "#6366f1", borderRadius: "8px", textDecoration: "none", fontWeight: 600, fontSize: "0.85rem" }}>
            ✏️ Edit Store
          </a>
        </div>
        ...
        {vendor?.store_banner && (
          <div style={{ position: "relative", marginBottom: "2rem", borderRadius: "12px" }}>
            <img src={vendor.store_banner} alt="Banner" style={{ width: "100%", height: "140px", objectFit: "cover", borderRadius: "12px", display: "block" }} />
            {vendor?.store_logo && (
              <img src={vendor.store_logo} alt="Logo" style={{ position: "absolute", bottom: "-20px", left: "1rem", width: "60px", height: "60px", borderRadius: "12px", objectFit: "cover", border: "3px solid white", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }} />
            )}
          </div>
        )}
      ...
      <div className="sd-card">
        ...
        <a href="/become-seller" style={{ display: "inline-block", marginTop: "1rem", padding: "0.5rem 1rem", background: "#eef2ff", color: "#6366f1", borderRadius: "8px", textDecoration: "none", fontWeight: 600, fontSize: "0.85rem" }}>
          🔄 Update Bank Details
        </a>
      </div>
    </div>
...
export const DashboardSkeleton = () => (
  <div className="sd-skeleton-wrap">
    <div className="sd-skeleton-sidebar">
      ...
      {[1,2,3,4,5,6].map((i) => <div key={i} className="sd-skeleton-row" />)}
    </div>
    <div className="sd-skeleton-main">
      <div className="sd-skeleton-topbar" />
      <div className="sd-skeleton-stats">
        {[1,2,3,4,5,6].map((i) => <div key={i} className="sd-skeleton-card" />)}
      </div>
...
export const DashboardError = ({ error, onRetry }) => (
  <div className="sd-error">
    ...
    <button className="sd-retry-btn" onClick={onRetry}>🔄 Try Again</button>
    <a href="/" className="sd-error-home">← Back to Home</a>
  </div>
);

// ── Backward compat (no duplicates) ──────────────────────────

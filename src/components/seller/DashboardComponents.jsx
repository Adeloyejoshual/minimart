...
// This avoids the import path resolution errors
...
export const Sidebar = ({
  vendor, activeSection, setActiveSection,
  sidebarOpen, setSidebarOpen, unreadCount,
}) => (
  <>
    {sidebarOpen && (
    ...
    <aside className={`sd-sidebar ${sidebarOpen ? "open" : ""}`}>
      <div className="sd-sidebar-header">
        ...
        <div className="sd-store-info">
          <h3 className="sd-store-name">{vendor?.store_name ?? "My Store"}</h3>
...
const QUICK_ACTIONS = [
  { icon: "➕", label: "Add Product", section: null,        href: "/minimart/add", color: "#6366f1", bg: "#eef2ff" },
  { icon: "📦", label: "Orders",      section: "orders",    href: null,            color: "#f59e0b", bg: "#fffbeb" },
  { icon: "💳", label: "Payouts",     section: "payouts",   href: null,            color: "#10b981", bg: "#ecfdf5" },
  { icon: "📈", label: "Analytics",   section: "analytics", href: null,            color: "#ec4899", bg: "#fdf2f8" },
];
...
export const Overview = ({ dash }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
    ...
    {/* Stat cards */}
    <StatCards stats={dash.stats} timeRange={dash.timeRange} setTimeRange={dash.setTimeRange} />

    {/* Chart + Notifications */}
    <div className="sd-grid-2">
      <RevenueChartCard data={dash.revenueChart} />
...
export const Payouts = ({ vendor }) => {
    ...
    const fetchAll = async () => {
      try {
        const [balRes, txRes, wdRes] = await Promise.all([
          axios.get("/api/seller-wallet/balance",      { headers: { Authorization: `Bearer ${token()}` } }),
          axios.get("/api/seller-wallet/transactions?limit=10", { headers: { Authorization: `Bearer ${token()}` } }),
          axios.get("/api/seller-wallet/withdrawals?limit=5",   { headers: { Authorization: `Bearer ${token()}` } }),
        ]);
  ...
  if (loading) {
    return <div className="sd-card" style={{ textAlign: "center", padding: "3rem", color: "#9ca3af" }}>
      Loading wallet...
    </div>;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      {virtualAccount ? (
        <div className="sd-card">
          ...
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "0.75rem 1rem", color: "#1e40af", fontSize: "0.82rem", marginTop: "1rem" }}>
            💡 Buyers pay directly to this account. Funds are credited automatically.
          </div>
        </div>
      ) : (
        <div className="sd-card">
      ...
      <div className="sd-card">
        ...
        {msg.text && (
          <div style={{
            padding: "0.75rem 1rem", borderRadius: "10px", fontSize: "0.875rem", marginTop: "1rem",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="sd-card">
        ...
        <a href="/become-seller"
          style={{ display: "inline-block", marginTop: "1rem", padding: "0.5rem 1rem", background: "#eef2ff", color: "#6366f1", borderRadius: "8px", textDecoration: "none", fontWeight: 600, fontSize: "0.85rem" }}>
          🔄 Update Bank Details
        </a>
      </div>
    </div>
...
export const StatCards      = ({ stats, timeRange, setTimeRange }) =>
  <Overview dash={{ stats, timeRange, setTimeRange, revenueChart: [], notifications: [], recentOrders: [], setActiveSection: () => {}, markNotifRead: () => {} }} />;
      ...
      <div className="sd-notif-list">
        {notifications.map((n) => (
          <div key={n.id} className={`sd-notif-item ${n.read ? "" : "unread"}`}
            onClick={() => !n.read && markNotifRead(n.id)} role="button" tabIndex={0}>
            <div className="sd-notif-dot" />
            <div className="sd-notif-content">
              <p className="sd-notif-text">{n.message}</p>
              <span className="sd-notif-time">{formatTimeAgo(n.created_at)}</span>
...
export const QuickActions = () => null; // merged into Overview
export const OrdersTable  = Orders;     // alias
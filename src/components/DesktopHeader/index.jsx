...
const UserDropdown = memo(function UserDropdown({
  ...
  const menuItems = [
    ...
    { icon: <Icons.orders />,    label: "My Orders",           path: "/shop/orders"           },
    ...
    >
      ...
      <div className="dh-dropdown__header" onClick={() => go("/profile")}>
        ...
        <div className="dh-dropdown__info">
          ...
          <p className="dh-dropdown__email">{user.email || user.store_name || "View Profile"}</p>
        </div>
      </div>
...
function DesktopHeader({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    // Messages
    axios
      .get(`${API}/conversations/unread-count`, { headers })
      .then((r) => setUnreadMsgs(Number(r.data?.count ?? r.data?.unread ?? 0)))
      .catch(() => {});

    // Saved
    axios
      .get(`${API}/saved/count`, { headers })
          ...
          {user ? (
            <>
              ...
              <IconBtn
                ...
                count={unreadMsgs}
                to="/conversations"
                active={location.pathname.startsWith("/conversations") || location.pathname.startsWith("/messages")}
              />

  ...
  return (
    <div className="hm-loc-bar">
      <button
        ...
        aria-label={
          label
            ? `Showing results in ${label}. Tap to change`
...
export default function Homepage({ user }) {
  ...
  /* ── Safely get cache context ─────────────────────────── */
  let cacheContext = { setProducts: () => {}, setLoaded: () => {} };
  try {
    cacheContext = useProductCache();
  } catch {}
  const { setProducts, setLoaded } = cacheContext;

  /* ── Location (renamed to avoid react-router clash) ────── */
  const {
    location,
    save  : saveLocation,
    clear : clearLocation,
  ...
  const { featured, deals, products } = useMemo(() => {
    try {
      ...
      const rest = allProducts.filter((p) => p && !p.is_promoted);

      return { featured: feat, deals: cheap, products: rest };
    } catch {
      return { featured: [], deals: [], products: allProducts };
    }
  }, [allProducts, data]);
    ...
    <div className="hm-root">
      ...
      <main className="hm-page" id="hm-main">
        ...
        {isError && (
          <ErrorBanner
            message={
              error?.message ?? "Could not reach the marketplace."
            }
            onRetry={refetch}
          />
        )}

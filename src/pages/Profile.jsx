...
export default function ProductDetail() {
  ...
  /* ── TRACK VIEW — once per browser session per product ── */
  useEffect(() => {
    if (!product?.id) return;
    const key = `viewed_${product.id}`;
    if (sessionStorage.getItem(key)) return; // already counted this session (survives refresh)

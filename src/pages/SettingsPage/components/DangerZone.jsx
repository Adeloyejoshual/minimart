...
════════════════════════════════════════════════════════════ */
function DeleteAccountModal({ open, onClose, onSuccess }) {
  ...
  const submitDelete = async () => {
    ...
    try {
      ...
      /* Give user time to read the success message, then hand off */
      setTimeout(() => onSuccess(), 2_600);
      ...
      <div className="dz-modal">
        ...
        <div className="dz-modal__header">
          <div className="dz-modal__title-wrap">
            <span className="dz-modal__alert"><AlertIcon /></span>
            <h3 className="dz-modal__title">Delete Account</h3>
          </div>
          {!loading && !done && (
            <button type="button" className="dz-modal__close"
        ...
        {!done ? (
          <>
            ...
            <div className="dz-modal__warning">
              <p className="dz-modal__warning-title">Please read this carefully</p>
              <ul className="dz-modal__warning-list">
                {DELETION_ITEMS.map((item, i) => <li key={i}>{item}</li>)}
              </ul>
            </div>
            ...
            {error && (
              <p className="dz-modal__error" role="alert">{error}</p>
            )}

            <div className="dz-modal__actions">
              <button type="button" className="dz-btn dz-btn--cancel"
                      onClick={onClose} disabled={loading}>
                Cancel
              </button>
              <button type="button" className="dz-btn dz-btn--danger"
                      onClick={submitDelete} disabled={!canSubmit}>
                {loading
                  ? <><span className="dz-spinner" aria-hidden="true" />Deleting…</>
                  : "Delete My Account"
                }
              </button>
            </div>
          ...
          <div className="dz-modal__success">
            ...
            <p className="dz-modal__success-text">
              Your account has been scheduled for deletion. It will be permanently
              removed after <strong>60 days</strong>. You can restore it by logging
              in before that date.
            </p>
            <p className="dz-modal__success-redirect">Redirecting to login…</p>
          </div>
        )}
      </div>
    </div>
...
export default function DangerZone({ settings }) {
  ...
  return (
    <>
      <SettingsSection title="Danger Zone" className="settings-section--danger">
        <div className="dz-card">
          ...
          >
            <div className="dz-trigger__left">
              <span className="dz-trigger__icon"><ActionsIcon /></span>
          ...
          >
            <div className="dz-panel__inner">
              ...
              >
                <span className="dz-action__icon"><TrashIcon /></span>
                ...
                <span className="dz-action__arrow"><ArrowRightIcon /></span>
              </button>

...
const cartApi = {
  ...
  clear: () =>
    axios.delete(CART_API, { headers: authHeaders() }),
  saveItem: (id) =>
...
export default function CartPage({ user }) {
  ...
  const totalSavings = useMemo(() => {
    return activeItems.reduce((s, i) => {
      const orig = Number(i.originalPrice ?? i.comparePrice ?? 0);
      const price = Number(i.price ?? 0);
      if (orig > price) return s + (orig - price) * i.qty;
      return s;
    }, 0);
  }, [activeItems]);
  ...
  const updateQty = useCallback((id, delta) => {
    ...
    setItems((prev) =>
      prev.map((item) => {
        ...
        const hasTracking =
          item.stock !== null && item.stock !== undefined && Number.isFinite(item.stock);
      ...
      {loading ? (
        ...
        <div className="ct-layout">
          <div className="ct-items-col">
            ...
            {/* Mobile-only inline checkout — hidden on desktop */}
            <div className="ct-inline-checkout">
              ...
              {totalSavings > 0 && (
                <p className="ct-inline-checkout-savings">
              ...
              >
                {checkingOut ? "Saving changes…" : !user ? "Login to Checkout" : hasOutOfStock ? "Remove unavailable items" : "Proceed to Checkout"}
              </button>
            </div>
      ...
      {!loading && items.length > 0 && (
        <div className="ct-sticky-bar">
          ...
          <button
            ...
            disabled={!canCheckout}
          >
            {checkingOut ? "Saving…" : !user ? "Login" : hasOutOfStock ? "Remove items" : "Checkout"}
          </button>
        </div>
      )}

...
export default function AddProduct() {
  ...
  const startPayment = async (productId, planId, amount, email) => {
    const res = await fetch("https://minimart-ivrm.onrender.com/api/payments/initialize", {
      ...
      body: JSON.stringify({
        email,
        amount,
        productId, // UUID from backend
        planId,    // INT8 from config
      }),
    });
  ...
  const handlePaymentSuccess = async (reference, idempotency_key) => {
    setLoading(true);
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save payment data for retry/callback
  ...
  useEffect(() => {
    ...
    if (status === 'success' && reference && paymentData) {
      ...
      showError("Payment cancelled. You can retry below.");
    }
  }, []);
  ...
  const showRetryButton = paymentData && !paymentSuccess;
      ...
      <div className="button-section section form-card">
        {showRetryButton ? (
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Processing..." : "Create & Activate Product"}
          </button>
        )}
      </div>

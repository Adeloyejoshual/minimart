...
const getImageUrl = (p) => {
  // 1. Pre-processed image field (backend should provide)
...
const splitProducts = (products) => ({
  ...
  trending: products.slice(0, 14), // Always show top products
    ...
    >
      ...
      <div className="co-grad">
        ...
        <div className="co-foot">
          <span className="co-loc">📍 {product.location?.city || "Nationwide"}</span>
          {product.distance_km != null && <span className="dist">{product.distance_km} km</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="ct-body">
        ...
        <div className="ct-loc">📍 {product.location?.city || "Nationwide"}</div>
        {product.seller?.verified && <div className="vfd">✓ Verified seller</div>}
    ...
    >
      ...
      <div className="feat-body">
        ...
        <div>
          ...
          <div className="feat-loc">📍 {product.location?.city || "Nationwide"}</div>
        </div>
      </div>
    </div>
  );
...
export default function Homepage({ user }) {
  ...
  const [sections, setSections] = useState({
    featured: [], nearby: [], trending: [], deals: [], latest: []
  });
    ...
    try {
      const data = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => 
          finish(() => fetchData().then(resolve).catch(reject)), 5000
        );
  ...
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === "visible" && !loading) loadHomepage();
    };
  ...
  // Derived Values
  const locationLabel = meta.location || (meta.nearbySource === "gps" ? "Near you" : null);
  const currentSections = category === "all" ? sections : sections; // Category filter disabled until backend fixed
  ...
  return (
    <>
      ...
      <div className="pg">
        ...
        {/* Global Empty State */}
        {!loading && !error && currentSections.latest.length === 0 && (
          <div className="empty">
        ...
        <div className="sec">
          ...
          {loading ? (
            ...
            <>
              <div className="grid2">
                {currentSections.deals.slice(0, dealsVisible).map((product) => (
                  <GridTile
                    key={product.id}
                    product={product}
                    onView={trackView}

 ...
 * v1 — Production
 * ─────────────────────────────────────────────────────
 * ✓ Real-time status timeline (visual progress)
 * ✓ Delivery ETA countdown
 * ✓ Full order breakdown (items, address, payment)
 * ✓ Rider contact info (when assigned)
 * ✓ Retry payment button for unpaid online orders
 * ✓ Cancel order button (only in early stages)
 * ✓ Contact seller / support links
 * ✓ Auto-refresh every 30s while active
 * ✓ Copy tracking ID
 * ✓ Share order link
 * ✓ Print / download receipt
 * ✓ Flat Jumia-style design matching checkout
 */
...
/* ═══════════════════════════════════════════════════════════════
   ORDER STATUS TIMELINE — the source of truth
═══════════════════════════════════════════════════════════════ */
const STATUS_STEPS = [
  {
    key      : "pending",
  ...
  },
  {
    ...
    subtitle : "Payment received — seller notified",
    codOnly  : false,
  },
  {
    key      : "processing",
  ...
  },
  {
    ...
    subtitle : "Loemart Express has your order",
    codOnly  : false,
  },
  {
    key      : "delivered",
...
/* For COD, "confirmed" step is "Order Confirmed" (no payment step) */
const COD_STATUS_STEPS = [
  {
    key      : "pending",
    label    : "Order Placed",
    subtitle : "Order confirmed — awaiting seller",
  },
  {
    key      : "confirmed",
  ...
  },
  {
    key      : "shipped",
    label    : "Out for Delivery",
    subtitle : "Rider is on the way — pay on arrival",
  },
  {
    key      : "delivered",
    label    : "Delivered",
...
/* Statuses that indicate the order is still in progress */
  ...
  return (
    <div className="otp-wrapper">
      <div className="otp-topbar">
        <button className="otp-back" aria-label="Back">
          <Icon.ArrowLeft />
        </button>
  ...
  return (
    <div className="otp-wrapper">
      <div className="otp-error-state">
        <div className="otp-error-state__icon">
          <Icon.Alert size={40} />
        </div>
        <h2 className="otp-error-state__title">Unable to load order</h2>
...
function StatusTimeline({ currentStatus, isCOD, timestamps = {} }) {
  ...
  /* Find current step index — treat "cancelled" specially */
      ...
      {!isCancelled && steps.map((step, idx) => {
        ...
        return (
          <div
            key={step.key}
            className={`otp-step ${
              isPast    ? "otp-step--past"    : ""
            } ${
              isCurrent ? "otp-step--current" : ""
            } ${
              isFuture  ? "otp-step--future"  : ""
            }`}
          >
            <div className="otp-step__marker">
              {isPast ? (
                ...
                <span className="otp-step__number">{idx + 1}</span>
              )}
            </div>
            ...
            <div className="otp-step__body">
              <div className="otp-step__label">
                {step.label}
                {isCurrent && (
                  <span className="otp-step__badge">In progress</span>
                )}
              </div>
              <p className="otp-step__subtitle">{step.subtitle}</p>
              {timestamp && (
                <p className="otp-step__time">
          ...
          {(subOrder.items ?? []).map((item) => (
            <div key={item.id} className="otp-item">
              ...
              <div className="otp-item__info">
                <p className="otp-item__name">
                  {item.product_name ?? "Product"}
                </p>
                {item.variant_name && (
                  <p className="otp-item__variant">{item.variant_name}</p>
                )}
                <p className="otp-item__qty">
                  Qty {item.quantity} × {fmt(item.price)}
                </p>
              </div>

              <p className="otp-item__price">
...
export default function OrderTracking() {
  ...
  const fetchOrder = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      console.error("[OrderTracking] fetch failed:", err.message);
      setError(
  ...
  /* Restart polling when order status changes */
  useEffect(() => {
    if (!order) return;
    ...
    /*
     * Only poll while order is still active.
     * No need to keep polling delivered/cancelled orders.
     */
  ...
  const handleShare = useCallback(async () => {
    ...
    const title = `My Loemart Order — ${order?.tracking_id ?? orderId}`;
  ...
  const handleCancelOrder = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.message ??
        "Could not cancel order. Contact support.",
        "error"
      );
    } finally {
      setCancelling(false);
    }
  }, [orderId, fetchOrder, showToast]);
  ...
  return (
    <div className="otp-wrapper">
      ...
      <div className="otp-content">
          ...
          <>
            ...
            <div className="otp-section-body">
              <div className="otp-delivery">
                <div className="otp-delivery__icon">
                  <Icon.Truck />
                </div>
                <div className="otp-delivery__body">
                  <p className="otp-delivery__title">Loemart Express</p>
                  <p className="otp-delivery__sub">
                    Delivered to your bus stop by trusted riders
                  </p>
                </div>
              </div>
              ...
              <div className="otp-address">
                <p className="otp-address__name">
                  ...
                  {order.phone && (
                    <span className="otp-address__phone"> · {order.phone}</span>
                  )}
                </p>
                <p className="otp-address__line">{order.address_line}</p>
                {order.landmark && (
                  <p className="otp-address__landmark">
                    <Icon.Pin /> {order.landmark}
                  </p>
                )}
                <p className="otp-address__city">
                  {order.city}, {order.state}
                </p>
                {order.call_before_delivery && (
                  <p className="otp-address__call">
        ...
        {/* ══ ITEMS ══ */}
        <div className="otp-section-header">
          <h2 className="otp-section-header__title">
            Items ({flatItemCount})
          </h2>
        </div>
        <div className="otp-section-body">
        ...
        <div className="otp-actions-section">
          ...
          <div className="otp-action-buttons">
            <a
              href={`/support?orderId=${trackingId}`}
            ...
            {order.orders?.[0]?.seller_id && (
              <Link
                to={`/messages?sellerId=${order.orders[0].seller_id}&orderId=${trackingId}`}
                className="otp-action-btn"
              >
                <Icon.MessageCircle />
            ...
            {canCancel && (
              <button
                type="button"
                className="otp-action-btn otp-action-btn--danger"
        ...
        >
          ...
          >
            ...
            <div className="otp-modal__actions">
              ...
              <button
                type="button"
                className="otp-btn otp-btn--danger"

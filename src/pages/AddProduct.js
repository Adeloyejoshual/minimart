...
const AddProduct = () => {
  ...
  return (
    <form onSubmit={handleAdd} style={{ maxWidth: 750, margin: 20, padding: 20, borderRadius: 10, background: "#fff", display: "flex", flexDirection: "column", gap: 15, boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
      ...
      {form.region && (
        <select value={form.stateLocation} onChange={e => handleChange("stateLocation", e.target.value)} required>
          <option value="">Select State</option>
          {allStates.map(s => <option key={s}>{s}</option>)}
        </select>
      )}
      {form.stateLocation && (
        <select value={form.cityLocation} onChange={e => handleChange("cityLocation", e.target.value)} required>

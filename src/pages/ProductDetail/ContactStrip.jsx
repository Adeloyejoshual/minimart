...
function ContactStrip({
  ...
  const hasContact = hasWA || hasPhone || hasSeller;

  if (!hasContact || isOwn) return null;

  // ✅ Consistent auth gate for all contact methods
  ...
  return (
    <div className="pd-contact-strip" role="group" aria-label="Contact seller">

      {/* Chat */}
      {hasSeller && (
        <button
          className="pd-btn pd-btn--chat"
          onClick={onChat}           // already has auth gate in parent
          disabled={chatBusy}
          aria-label={chatBusy ? "Opening chat…" : "Chat with seller"}
          aria-busy={chatBusy}
        >
          {chatBusy ? <span className="pd-spinner" aria-hidden="true" /> : <ChatIcon />}
      ...
      {hasWA && (
        <button
          ...
          onClick={requireAuth(onWhatsApp)}  // ✅ now guarded
      ...
      {hasPhone && (
        <button
          ...
          onClick={requireAuth(onCall)}       // ✅ now guarded

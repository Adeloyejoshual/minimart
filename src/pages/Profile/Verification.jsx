 ...
 * Step 1 — Email (OTP)
 * Step 2 — Identity (government ID + selfie)
...
═══════════════════════════════════════════════════════════════ */
const OTP_LENGTH        = 6;
const OTP_AUTO_DELAY_MS = 150;
const RESEND_SECONDS    = 30;
const MAX_DOC_MB        = 5;
const MAX_LOGO_MB       = 2;
const MAX_DOC_NUMBER    = 30;
...
const DOC_TYPES = [
  {
    ...
    frontLabel : "Upload NIN Slip (Front)",
    backLabel  : "Upload NIN Slip (Back)",
  },
  {
    value      : "passport",
    label      : "International Passport",
    numberLabel: "Passport Number",
    frontLabel : "Upload Passport Photo Page",
    backLabel  : "Upload Passport Back Page",
  },
  {
    value      : "drivers_license",
    label      : "Driver's License",
    numberLabel: "License Number",
    frontLabel : "Upload License (Front)",
    backLabel  : "Upload License (Back)",
  },
  {
    value      : "voters_card",
    label      : "Voter's Card",
    numberLabel: "VIN",
    frontLabel : "Upload Voter's Card (Front)",
    backLabel  : "Upload Voter's Card (Back)",
  },
];
...
const isSuccessMsg = (msg = "") =>
  msg.toLowerCase().includes("submit") ||
...
function OTPInput({ length = OTP_LENGTH, value, onChange, disabled, hasError }) {
  const refs = useRef([]);
      ...
      <div className="selfie-preview">
        {preview ? (
          ...
          <div className="selfie-placeholder">
            <User size={36} /><span>No photo</span>
          </div>
        )}
      </div>
...
export default function Verification() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      console.error("[Verification] fetchStatus:", err.message);
    } finally {
  ...
  /* ════════════════════════════════════════════════════════════
     SEND OTP
     Opens OTP area immediately — sends in background
  ════════════════════════════════════════════════════════════ */
  ...
  const verifyOTP = useCallback(async () => {
    if (otp.length < OTP_LENGTH || verifyingRef.current || verifying) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus((p) => ({
          ...p,
          email_verified : true,
          trust_score    : data.trust_score ?? ((p?.trust_score || 0) + 30),
        }));
        fetchStatus();
      } else {
        setHasOtpError(true);
  ...
  useEffect(() => {
    if (otp.length === OTP_LENGTH && emailStep === "otp" && !verifying && !verifyingRef.current) {
      const t = setTimeout(() => verifyOTP(), OTP_AUTO_DELAY_MS);
    ...
    <div className="verification-page">
      <div className="verification-container">
        ...
        <motion.div
          layout
          className={`step-item ${
            emailStep === "done" ? "step-item--complete" :
            emailStep === "otp"  ? "step-item--active"   :
            ""
          }`}
        >
          <div className="step-item-header">
            ...
            <div className="step-item-action">
              {emailStep === "done" ? (
                ...
                <button className="v-btn v-btn--primary v-btn--small" onClick={sendOTP}>
              ...
              >
                ...
                {devOtp && (
                  <div style={{
                    ...
                    Test code:{" "}
                    <strong style={{ letterSpacing: 4, fontSize: 18 }}>{devOtp}</strong>
                  </div>
                )}
                  ...
                  <div>
                    {resendRemaining <= 0 ? (
                      ...
                      <div className="resend-countdown">
                        <span>Resend in</span>
                        <Countdown seconds={RESEND_SECONDS} onComplete={() => setCanResend(true)} />
                      </div>
                    )}
                  </div>
                  <div className="security-note">
                    <Lock size={11} /><span>Do not share</span>
                  </div>
                </div>
              </motion.div>
            )}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <FileUpload
                          ...
                          hint={`JPG, PNG or PDF — max ${MAX_DOC_MB}MB`}
                          accept="image/*,.pdf"
                          file={docFront}
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 12, color: "#4b5563", marginBottom: 10 }}>
                          Clear photo of your face. Must match your ID.
                        </p>
                        <SelfieCapture
                          file={selfie}
                      ...
                      {/* Feedback */}
                      {idMsg && (
                        <div className={isSuccessMsg(idMsg) ? "v-success-msg" : "v-error"}>
                          {isSuccessMsg(idMsg)
                            ? <CheckCircle size={14} />
                            : <XCircle size={14} className="v-error-icon" />
                          }
                      ...
                      {/* Submit */}
                      <button
                        className={`v-btn v-btn--full ${
                          docFront && docBack && selfie && docNumber.trim().length >= 4
                            ? "v-btn--primary"
                            : "v-btn--ghost"
                        }`}
            ...
            <div className="step-body">
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <FileUpload
                  ...
                  hint={`JPG or PNG — max ${MAX_LOGO_MB}MB`}
                  accept="image/*"
                  file={storeLogo}
                ...
                {storeMsg && (
                  <div className={isSuccessMsg(storeMsg) ? "v-success-msg" : "v-error"}>
                    {isSuccessMsg(storeMsg)
                      ? <CheckCircle size={14} />
                      : <XCircle size={14} className="v-error-icon" />
                    }
        ...
        <div className="submit-bar">
          <div className="submit-bar-progress">
            ...
            <span className="submit-bar-count">{completedCount}/3</span>
          </div>

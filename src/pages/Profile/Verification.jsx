...
import {
  ...
  RefreshCw, Lock, BadgeCheck, Upload, Camera,
  FileText, User, X, Image, CreditCard,
} from "lucide-react";
      ...
      {Array.from({length}).map((_,i) => (
        <input key={i} ref={el=>(refs.current[i]=el)}
          ...
          onChange={e=>{
            ...
            const a=value.split("");
            if(!v){a[i]="";onChange(a.join(""));if(i>0)refs.current[i-1]?.focus();return}
            a[i]=v.slice(-1);onChange(a.join(""));
  ...
  if(file) return (
    <div className="upload-area upload-area--has-file">
      <div className="upload-preview">
        ...
        <div className="upload-preview-info"><p className="upload-preview-name">{file.name}</p><p className="upload-preview-size">{formatSize(file.size)}</p></div>
...
export default function Verification() {
  ...
  const fetchStatus = useCallback(async()=>{
    try{
      ...
      if(res.ok){
        ...
        else if(data.email) setUserEmail(data.email);
  ...
  const verifyOTP = useCallback(async()=>{
    ...
    setVerifying(true);setEmailError("");setHasOtpError(false);
    ...
    try{
      ...
      if(res.ok){
        setEmailStep("done");
        setDevOtp("");
        setStatus(p=>({...p,email_verified:true,trust_score:data.trust_score??(p?.trust_score||0)+30}));
        fetchStatus();
      }else{
        setHasOtpError(true);setOtp("");
  ...
  const submitIdentity = async()=>{
    ...
    try{
      const res=await fetch(`${API}/verification/submit-identity`,{method:"POST",headers:{Authorization:`Bearer ${localStorage.getItem("token")}`},body:fd});
      const data=await res.json();
  ...
  const submitStore = async()=>{
    ...
    try{
      const res=await fetch(`${API}/verification/submit-store`,{method:"POST",headers:{Authorization:`Bearer ${localStorage.getItem("token")}`},body:fd});
      const data=await res.json();
      ...
      <div className="verification-container">
                ...
                {emailError&&(
                  <div className="v-error">
                    ...
                    <div>
                      ...
                      {attemptsLeft<10&&attemptsLeft>0&&(
                ...
                <div className="resend-row">
                  <div>
                    {resendRemaining<=0?<span style={{fontSize:13,color:"#4b5563"}}>Daily limit reached</span>:
                     canResend?<button className="v-btn v-btn--link" onClick={sendOTP} disabled={sending}>
                       <RefreshCw size={12} className={sending?"v-spin":""}/> Resend ({resendRemaining} left)
                     </button>:
                     <div className="resend-countdown"><span>Resend in</span><Countdown seconds={30} onComplete={()=>setCanResend(true)}/></div>}
                  </div>
        ...
        <div className={`step-item ${identityVerified?"step-item--complete":""}`}>
          <div className="step-item-header">
            ...
            <div className="step-item-info">
              ...
              {identityVerified?<p className="step-item-verified"><CheckCircle size={12}/> Verified</p>:
               idReview?.status==="pending"?<p className="step-item-desc">Under review</p>:
               idReview?.status==="rejected"?<p className="step-item-desc" style={{color:"#f87171"}}>Rejected — resubmit</p>:
               <p className="step-item-desc">Government-issued ID + selfie</p>}
            </div>
            <Chip status={identityVerified?"complete":idReview?.status==="pending"?"in_review":idReview?.status==="rejected"?"rejected":"pending"}/>
          </div>
          ...
          {!storeVerified&&storeReview?.status!=="pending"&&(
            <div className="step-body">
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div><label className="v-field-label">Store Name</label><input type="text" className="v-input" value={storeName} onChange={e=>setStoreName(e.target.value)} placeholder="Enter store name" maxLength={60}/></div>
                <div><label className="v-field-label">Store Description</label><textarea className="v-textarea" value={storeDesc} onChange={e=>setStoreDesc(e.target.value)} placeholder="What do you sell?" maxLength={300} rows={3}/></div>
                ...
                {storeMsg&&(
                  <div className={storeMsg.toLowerCase().includes("submit")||storeMsg.toLowerCase().includes("review")?"v-success-msg":"v-error"}>
                    {storeMsg.toLowerCase().includes("submit")||storeMsg.toLowerCase().includes("review")?<CheckCircle size={14}/>:<XCircle size={14} className="v-error-icon"/>}
                    <span style={{fontSize:13}}>{storeMsg}</span>
                  </div>
                )}

                <button className={`v-btn v-btn--full ${storeName.trim()?"v-btn--primary":"v-btn--ghost"}`}
                  disabled={!storeName.trim()||storeSubmitting} onClick={submitStore}>
                  {storeSubmitting?<><Loader2 size={14} className="v-spin"/> Submitting</>:<><Store size={14}/> Submit Store</>}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="submit-bar">
          <div className="submit-bar-progress">
            ...
            <span className="submit-bar-count">{[emailVerified,identityVerified,storeVerified].filter(Boolean).length}/3</span>
          </div>
          <div className="submit-bar-track">
            <motion.div className="submit-bar-fill" initial={{width:0}}

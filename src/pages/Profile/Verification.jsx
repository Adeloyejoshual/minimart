...
══════════════════════════════════════════════════════════════════════════════ */
const DOC_TYPES = [
  { value: "nin",             label: "National ID (NIN)",       fields: ["front"],         numberLabel: "NIN Number" },
  { value: "passport",        label: "International Passport",  fields: ["front"],         numberLabel: "Passport Number" },
  { value: "drivers_license", label: "Driver's License",        fields: ["front", "back"], numberLabel: "License Number" },
  { value: "voters_card",     label: "Voter's Card",            fields: ["front"],         numberLabel: "VIN" },
];
      ...
      {Array.from({length}).map((_,i)=>(
        <input key={i} ref={el=>(refs.current[i]=el)}
          ...
          onChange={e=>{
            ...
            set(i,v.slice(-1));if(i<length-1)refs.current[i+1]?.focus();
          }}
          onKeyDown={e=>{if(e.key==="Backspace"&&!value[i]&&i>0)refs.current[i-1]?.focus()}}
          onPaste={e=>{e.preventDefault();const p=e.clipboardData.getData("text").replace(/\D/g,"").slice(0,length);onChange(p.padEnd(length,"").slice(0,length))}}
        />
      ))}
    </div>
  );
...
function FileUpload({ label, hint, accept, file, onFileChange, onRemove }) {
  ...
  return (
    <label className="upload-area">
      ...
      <Upload size={24} className="upload-icon"/><p className="upload-text">{label}</p><p className="upload-hint">{hint}</p>
    </label>
  );
}
...
export default function Verification() {
  ...
  const needsBack       = selectedDoc?.fields.includes("back");

  const emailDone  = emailVerified;
  const idDone     = identityVerified;
  const selfieDone = !!selfie || identityVerified;
  const storeDone  = storeVerified;

  const completed = [emailDone,idDone||!!(docFront&&selfie&&docType),storeDone||!!(storeName.trim()&&storeLogo)].filter(Boolean).length;
  const total     = 3;

  /* ── Send OTP ─────────────────────────────────────────────────────────────── */
  const sendOTP = useCallback(async()=>{
    setEmailStep("sending");setSending(true);setError("");setOtp("");setHasError(false);
    try{
      const res=await fetch(`${API}/verification/send-email-otp`,{method:"POST",headers:authHeaders()});
      const data=await res.json();
      if(res.ok){setEmailStep("otp");setCanResend(false);if(typeof data.remaining==="number")setResendRemaining(data.remaining)}
      else{setError(data.message||"Failed.");setEmailStep("idle");if(res.status===429&&data.remaining===0)setResendRemaining(0)}
    }catch{setError("Network error.");setEmailStep("idle")}
  ...
  const verifyOTP = useCallback(async()=>{
    ...
    verifyingRef.current=true;setVerifying(true);setError("");setHasError(false);
    try{
      const res=await fetch(`${API}/verification/verify-email-otp`,{method:"POST",headers:authHeaders(),body:JSON.stringify({otp})});
      const data=await res.json();
      if(res.ok){setEmailStep("done");setStatus(p=>({...p,email_verified:true,trust_score:data.trust_score??(p?.trust_score||0)+30}));fetchStatus()}
      else{setHasError(true);setOtp("");setError(data.message||"Invalid.");if(typeof data.attemptsLeft==="number")setAttemptsLeft(data.attemptsLeft);setTimeout(()=>setHasError(false),600)}
  ...
  useEffect(()=>{
    if(otp.length===6&&emailStep==="otp"&&!verifying&&!verifyingRef.current){
      const t=setTimeout(()=>verifyOTP(),150);return()=>clearTimeout(t);
    }
  ...
  const submitIdentity = async()=>{
    ...
    try{
      ...
      const data=await res.json();
      setIdMsg(data.message||"Submitted.");
  ...
  const submitStore = async()=>{
    ...
    try{
      ...
      const data=await res.json();
      setStoreMsg(data.message||"Submitted.");
      if(res.ok) fetchStatus();
    }catch{setStoreMsg("Network error.")}
    finally{setStoreSubmitting(false)}
  };

  if(pageLoading) return <div className="v-loading"><Loader2 size={26} className="v-spin"/></div>;
    ...
    <div className="verification-page">
      <div className="verification-container">
        ...
        {/* ════════════════════════════════════════════════════════════════════
            STEP 1 — EMAIL
        ════════════════════════════════════════════════════════════════════ */}
        <motion.div layout className={`step-item ${emailDone?"step-item--complete":emailStep!=="idle"?"step-item--active":""}`}>
          <div className="step-item-header">
            ...
            <div className="step-item-info">
              ...
              {emailDone?(
                <p className="step-item-verified">
                  ...
                  {status?.email_verified_at&&<span style={{color:"#6b7280",marginLeft:4}}>
            ...
            <div className="step-item-action">
              {emailDone?<Chip status="complete"/>:
               emailStep==="idle"?<button className="v-btn v-btn--primary v-btn--small" onClick={sendOTP} disabled={sending}><Mail size={13}/> Verify Email</button>:
          ...
          <AnimatePresence>
            {emailStep==="otp"&&(
              <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} transition={{duration:0.25}} className="step-body otp-section">
                <div className="otp-section-header">
                  ...
                  <p className="otp-section-subtitle">6-digit code sent to <span className="otp-section-email">{status?.email}</span></p>
                </div>
                <OTPInput length={6} value={otp} onChange={setOtp} disabled={verifying} hasError={hasError}/>
                <p className="otp-helper">Auto-submit enabled</p>
                {verifying&&<div className="verifying-indicator"><Loader2 size={14} className="v-spin"/><span>Verifying</span></div>}
                {error&&<div className="v-error"><XCircle size={14} className="v-error-icon"/><div><p className="v-error-text">{error}</p>{attemptsLeft<5&&attemptsLeft>0&&<p className="v-error-sub">{attemptsLeft} attempt{attemptsLeft!==1?"s":""} left</p>}</div></div>}
                <div className="resend-row">
                  <div>
                    {resendRemaining<=0?<span style={{fontSize:13,color:"#4b5563"}}>Daily limit reached</span>:
                     canResend?<button className="v-btn v-btn--link" onClick={sendOTP} disabled={sending}><RefreshCw size={12} className={sending?"v-spin":""}/> Resend ({resendRemaining} left)</button>:
                     <div className="resend-countdown"><span>Resend in</span><Countdown seconds={60} onComplete={()=>setCanResend(true)}/></div>}
                  </div>
                  <div className="security-note"><Lock size={11}/><span>Do not share</span></div>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {/* ════════════════════════════════════════════════════════════════════
            STEP 2 — IDENTITY DOCUMENT + SELFIE
        ════════════════════════════════════════════════════════════════════ */}
        <div className={`step-item ${idDone?"step-item--complete":""}`}>
          <div className="step-item-header">
            <div className={`step-item-icon ${idDone?"step-item-icon--complete":"step-item-icon--id"}`}>
              {idDone?<CheckCircle size={20}/>:<CreditCard size={20}/>}
            </div>
            <div className="step-item-info">
          ...
          {!idDone&&idReview?.status!=="pending"&&(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {DOC_TYPES.map(dt=>(
                      <label key={dt.value} className={`step-item ${docType===dt.value?"step-item--active":""}`}
                        style={{cursor:"pointer",padding:"12px 14px",marginBottom:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <input
                            type="radio" name="docType"
                            value={dt.value}
                            checked={docType===dt.value}
                            onChange={()=>{setDocType(dt.value);setDocFront(null);setDocBack(null);setDocNumber("")}}
                ...
                <AnimatePresence>
                  {docType&&selectedDoc&&(
                    ...
                    >
                      ...
                      {/* Front upload */}
                      <FileUpload
                        label={`Upload ${selectedDoc.label} ${needsBack?"(Front)":""}`}
                      ...
                      {needsBack&&(
                        <FileUpload
                          label={`Upload ${selectedDoc.label} (Back)`}
                          hint="JPG, PNG or PDF — max 5MB"
                          accept="image/*,.pdf"
                          file={docBack}
                      ...
                      {/* Submit identity */}
                      {idMsg&&(
                        <div className={idMsg.includes("submitted")?"v-success-msg":"v-error"}>
                          {idMsg.includes("submitted")?<CheckCircle size={14}/>:<XCircle size={14} className="v-error-icon"/>}
                          <span style={{fontSize:13}}>{idMsg}</span>
                        </div>
                      )}

                      <button
                        className={`v-btn v-btn--full ${docFront&&selfie&&docNumber.trim()?"v-btn--primary":"v-btn--ghost"}`}
                        disabled={!docFront||!selfie||!docNumber.trim()||(needsBack&&!docBack)||idSubmitting}
          ...
          <div className="step-item-header">
            ...
            <div className="step-item-info">
              ...
              <p className="step-item-desc">
                {storeVerified?"Verified":storeReview?.status==="rejected"?"Rejected — resubmit":storeReview?.status==="pending"?"Under review":"Set up your store"}
              </p>
            </div>
            <Chip status={storeVerified?"complete":storeReview?.status==="rejected"?"rejected":storeReview?.status==="pending"?"in_review":"pending"}/>
          </div>
          ...
          {!storeVerified&&storeReview?.status!=="pending"&&(
            <div className="step-body">
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {storeMsg&&(
                  <div className={storeMsg.includes("submitted")||storeMsg.includes("review")?"v-success-msg":"v-error"}>
                    {storeMsg.includes("submitted")||storeMsg.includes("review")?<CheckCircle size={14}/>:<XCircle size={14} className="v-error-icon"/>}
                ...
                <button
                  ...
                  onClick={submitStore}
                >
                  {storeSubmitting?<><Loader2 size={14} className="v-spin"/> Submitting</>:<><Store size={14}/> Submit Store for Review</>}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="submit-bar">
          <div className="submit-bar-progress">
            ...
            <span className="submit-bar-count">
              {[emailDone,idDone,storeDone].filter(Boolean).length}/3
            </span>
          </div>
          <div className="submit-bar-track">
            <motion.div className="submit-bar-fill"
              initial={{width:0}}
              animate={{width:`${([emailDone,idDone,storeDone].filter(Boolean).length/3)*100}%`}}

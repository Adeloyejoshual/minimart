...
const StatusBadge = memo(({ status, isActive }) => {
  const label =
    isActive && (status === "active" || status?.startsWith("active_"))
      ...
      : status?.startsWith("pending_") || status === "pending" ? "Pending"
      : status || "Unknown";
...
/* ═══════════════════════════════════════════════════════════════
   PRODUCT ROW
═══════════════════════════════════════════════════════════════ */
const ProductRow = memo(function ProductRow({ product, onEdit, onDelete, onToggle, isDeleting }) {
  ...
  return (
    <div className={`db-prod-row${isDeleting ? " db-prod-row--del" : ""}`}>
      ...
      <div className="db-prod-actions">
        <button className="db-act db-act--edit" onClick={() => onEdit(product)} title="Edit">
          <Ic.Edit />
        </button>
        <button
        ...
        <button className="db-act db-act--delete" onClick={() => onDelete(product)} title="Delete">
          <Ic.Trash />
        </button>
      </div>
    </div>
...
export default function Dashboard({ user }) {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      console.log("[dashboard] stats:", res.status, d);
      if (res.ok && d.success) {
        setStats(d.stats);
      } else {
        console.warn("[dashboard] stats failed:", d.message);
      }
    } catch (err) {
  ...
  const loadProducts = useCallback(async (currentTab = "all") => {
    ...
    try {
      ...
      const d = await res.json();
      console.log("[dashboard] products:", res.status, d);

      if (!res.ok) {
        showToast(d.message || d.detail || `Error ${res.status}`, "error");
        return;
      }
  ...
  const loadAnalytics = useCallback(async () => {
    try {
      ...
      const d = await res.json();
      console.log("[dashboard] analytics:", res.status, d);
      if (res.ok && d.success) {
        setAnalytics(d);
      }
    } catch (err) {
      console.error("[dashboard] loadAnalytics error:", err);
    }
  }, []);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/seller-dashboard/products/${product.id}`, {
        method: "DELETE",
        headers: authH(),
      });
      const d = await res.json();
  ...
  const handleToggle = useCallback(async (product) => {
    try {
      const res = await fetch(`${API}/seller-dashboard/products/${product.id}/toggle`, {
        method: "PATCH",
        headers: authH(),
      });
      const d = await res.json();
      ...
      if (res.ok && d.success) {
        setProducts((prev) =>
          prev.map((p) =>
            p.id === product.id ? { ...p, is_active: d.is_active, status: d.status } : p
          )
        );
        loadStats();
        showToast(d.is_active ? "Listing activated ✓" : "Listing paused", d.is_active ? "success" : "info");
      } else {
  ...
  const tabCounts = useMemo(() => ({
    ...
    active: stats?.active ?? products.filter((p) => (p.status === "active" || p.status?.startsWith("active_")) && p.is_active).length,
    draft: stats?.draft ?? products.filter((p) => p.status === "draft").length,
    paused: stats?.paused ?? products.filter((p) => !p.is_active && p.status !== "draft" && p.status !== "deleted").length,
    pending: stats?.pending ?? products.filter((p) => p.status?.startsWith("pending") || p.status === "pending").length,
  }), [stats, products]);
  ...
  const insights = useMemo(() => {
    ...
    if (tabCounts.active === 0)
      list.push({ icon: "⚠️", msg: "You have no active listings — activate or create one.", type: "warn" });
    if (tabCounts.draft > 0)
      list.push({ icon: "📝", msg: `You have ${tabCounts.draft} draft${tabCounts.draft > 1 ? "s" : ""} — publish them.`, type: "info" });
    if (tabCounts.pending > 0)
      list.push({ icon: "⏳", msg: `${tabCounts.pending} listing${tabCounts.pending > 1 ? "s" : ""} awaiting approval.`, type: "info" });
    if (tabCounts.active >= 5)
      list.push({ icon: "🎉", msg: `${tabCounts.active} active listings — great job!`, type: "good" });
    if ((stats.total_views || 0) > 100)
      list.push({ icon: "📈", msg: `${fmtNum(stats.total_views)} total views — promote top listings!`, type: "good" });
    return list;
  }, [stats, tabCounts]);
    ...
    <div className="db-page">
      ...
      <div className="db-topbar">
        ...
        <div className="db-topbar-right">
          ...
          <button className="db-topbar-btn" onClick={() => navigate("/notifications")} title="Notifications"><Ic.Bell /></button>
        </div>
      </div>
      ...
      <div className="db-scroll">
          ...
          <>
            ...
            {loading ? <StatsSkeleton /> : stats ? (
              <div className="db-stats-grid">
                ...
                <StatCard icon={<Ic.Naira />} label="Total Revenue" value={naira(stats.total_revenue)} sub={stats.rating > 0 ? `⭐ ${Number(stats.rating).toFixed(1)} rating` : "No sales yet"} color="#e8630a" />
              </div>
            ) : null}
            ...
            {!loading && products.length > 0 && (
              <div className="db-card">
                ...
                {products.slice(0, 3).map((p) => (
                  <ProductRow key={p.id} product={p} onEdit={handleEdit} onDelete={handleDelete} onToggle={handleToggle} isDeleting={deleting === p.id} />
                ))}
              </div>
            )}
            ...
            {!loading && products.length === 0 && (
              <div className="db-card">
                <div className="db-empty">
                  <span>🛍️</span>
                  <p>No listings yet</p>
                  <button onClick={() => navigate("/minimart/add")}>Post your first listing →</button>
                </div>
              </div>
            )}
        ...
        {section === "analytics" && (
          <>
            ...
            {stats && (
              <div className="db-card">
                <h2 className="db-card-title" style={{ marginBottom: 14 }}>📦 Listing Breakdown</h2>
            ...
            {!analytics && !loading && (
              <div className="db-card">
                <div className="db-empty">
                  ...
                  <small style={{ color: "#aaa", fontSize: 12 }}>Start getting views to see your analytics</small>
                </div>
              </div>
            )}
          </>

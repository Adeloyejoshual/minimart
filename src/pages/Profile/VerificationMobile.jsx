...
const Ic = {
  ...
  Loader: ({ s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="v-spin">
...
const getTier = (score) => TIERS.find(([min]) => score >= min)?.[1] || "Starter";
  ...
  return (
    <div className="v-field">
      ...
      {value ? (
        ...
        <div
          className={["v-upload", drag ? "v-upload--drag" : "", error ? "v-upload--error" : ""].filter(Boolean).join(" ")}
          role="button"
          tabIndex={0}
        ...
        >
          <Ic.Upload s={22} />
          <span className="v-upload__label">{drag ? "Drop here" : "Click or drag to upload"}</span>
...
function SelfieCapture({ value, onChange, error }) {
  ...
  return (
    <div className="v-selfie">
      ...
      <p className="v-selfie__guide">Face forward in good lighting. Remove glasses, hats and masks.</p>
      ...
      <div className="v-selfie__actions">
        <input
          ...
          onChange={(e) => handleFile(e.target.files[0])}
        />
        <button type="button" className="v-btn v-btn--ghost v-btn--sm" onClick={() => fileRef.current?.click()}>
    ...
    <div className="v-checklist">
      {items.map(({ label, done }) => (
        <div key={label} className={`v-checklist__row${done ? " v-checklist__row--done" : ""}`}>
          <div className="v-checklist__dot">{done && <Ic.Check s={9} />}</div>
          {label}
        </div>
      ))}
    </div>
    ...
    <div className="v-email-gate">
      ...
      {(phase === "sent" || phase === "verifying") && (
        <div className="v-otp-panel">
          <p className="v-otp-panel__dest">Enter the 6-digit code — auto-submits when complete</p>
          ...
          {expiresIn > 0 && (
            <div className="v-otp-panel__hint">
              <Ic.Clock s={12} /> Expires in <Countdown seconds={expiresIn} onExpire={() => setExpiresIn(0)} />
            </div>
          )}
          ...
          <div className="v-resend-row">
            ...
            {cooldown > 0 ? (
              <div className="v-resend-row__timer">
                <Ic.Clock s={12} /> Resend in <Countdown seconds={cooldown} onExpire={() => setCooldown(0)} />
              </div>
...
function VerificationForm({ onSuccess }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (result.ok) {
        ...
        setFaceResult({ skipped: true, error: result.message });
      }
    })();
  ...
  const validate = () => {
    ...
    if (!form.docType) {
      e.docType = "Select a document type.";
      ok = false;
    }
    if (!form.docNumber.trim()) {
      e.docNumber = "Enter your document number.";
      ok = false;
    } else if (form.docType && !DOC_VALIDATORS[form.docType]?.(form.docNumber)) {
      e.docNumber = `Invalid ${form.docType.replace(/_/g, " ")} number.`;
      ok = false;
    }
    if (!form.front) {
      e.front = "Document front is required.";
      ok = false;
    }
    if (!form.back) {
      e.back = "Document back is required.";
      ok = false;
    }
    if (!form.selfie) {
      e.selfie = "Selfie is required.";
      ok = false;
    }
  ...
  return (
    <form className="v-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="v-form__section">
        ...
        {faceResult && !faceLoading && (
          faceResult.skipped ? (
            ...
            <Alert type="error">Selfie does not match document. Please retake both photos.</Alert>
          )
        )}
      ...
      <div className="v-form__section">
        ...
        <Alert type="info">
          Optionally upload a store logo or supporting business document. It will be stored securely for admin review.
        </Alert>
      ...
      {!allDone && !submitting && (
        <p className="v-form__incomplete-hint">Complete all required fields above to submit.</p>
      )}
    </form>
  );
}
...
function StatusCard({ identityReview, storeReview, onResubmit }) {
  ...
  const combined =
    idStatus === "approved" && storeStatus === "approved"
      ? "approved"
      : idStatus === "rejected" || storeStatus === "rejected"
      ? "rejected"
      : "pending";
  ...
  const BODY = {
    ...
    rejected:
      identityReview?.rejection_reason ?? storeReview?.rejection_reason ?? "Please check the reason and resubmit.",
  };
...
function StatusChip({ status }) {
  ...
  const cls =
    identity_verified && store_verified
      ? "chip--complete"
      : identity_review?.status === "pending"
      ? "chip--review"
      : identity_review?.status === "rejected"
      ? "chip--rejected"
      : email_verified
      ? "chip--active"
      : "chip--pending";
...
export default function Verification() {
  ...
  const progressDone = status
    ? [status.email_verified, !!status.identity_review, !!(status.identity_verified && status.store_verified)].filter(
        Boolean
      ).length
    : 0;
    ...
    return (
      <div className="v-page">
        <div className="v-page-error">
          ...
          <button className="v-btn v-btn--ghost" onClick={fetchStatus} style={{ marginTop: 12 }}>
          ...
          {view === "submitted" && (
            <div className="v-status-card v-status-card--pending">
              ...
              <p className="v-status-card__body">
                Our team will review your documents within 24 hours. You'll be notified once a decision is made.
              </p>
            </div>
          )}

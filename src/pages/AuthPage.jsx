 ...
 * - Nigeria location picker (state + city)
...
/* ── Icon resolver (for data-driven lists) ── */
const IcMap = {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      particles.current = Array.from(
        { length: Math.min(35, Math.floor((w * h) / 16_000)) },
        () => ({
          x  : Math.random() * w,
          y  : Math.random() * h,
...
export default function AuthPage({ setUser }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.error("[AuthPage] login:", err);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.error("[AuthPage] register:", err);
        ...
        <div className="ap-left-inner">
          ...
          <div className="ap-logo">
            ...
            <span className="ap-logo-name">
              {BRAND_NAME.slice(0, 3)}<b>{BRAND_NAME.slice(3)}</b>
            </span>
          </div>
            ...
            <div className="ap-feats">
              {FEATURES.map((f) => (
                <div className="ap-feat" key={f.title}>
                  <div className="ap-feat-icon">
                    {IcMap[f.icon]?.(BRAND_COLOR)}
                  </div>
          ...
          <div className="ap-trust-grid">
            {TRUST_ITEMS.map((t) => (
              <div className="ap-trust-item" key={t.label}>
                <div className="ap-trust-ic">
                  {IcMap[t.icon]?.(BRAND_COLOR)}
                </div>
                <div>

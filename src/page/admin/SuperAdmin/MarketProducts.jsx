    ...
    <div className="overlay" onClick={onClose}>
      ...
      >
        ...
        <p style={{ fontSize: ".82rem", color: "#888", marginBottom: 8 }}>
          This will soft-delete <strong>{product.name}</strong>. The seller will
          be notified.
        </p>
...
function ProductDrawer({
  ...
  const isPending =
    product.status === "pending_review" || product.status === "flagged";
        ...
        <div style={{ padding: 20, flex: 1 }}>
          ...
          >
            ...
            {product.is_sponsored && (
              <FlagChip label="Sponsored" color="#9333ea" />
            )}
              ...
              >
                <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                  <span style={{ fontSize: 22, fontWeight: 900, color: "#ff5722" }}>
                    {Number(product.base_price ?? product.price ?? 0).toLocaleString(
                      "en-NG",
                      {
                        style: "currency",
                        currency: "NGN",
...
export default function MarketProducts({ confirm }) {
  ...
  const handleReject = useCallback(
    async (id, reason) => {
      setBusy(`rp-${id}`);
      try {
        await adminApi.post(`/products/${id}/reject`, {
          ...
          >
            <thead>
              <tr style={{ borderBottom: "2px solid #f0eeea" }}>
                {["", "Product", "Seller", "Price", "Status", "Flags", "Fraud", "Date", "Actions"].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
              ...
              {displayed.map((p) => {
                ...
                return (
                  <tr
                    ...
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.background = p.is_flagged ? "#fffbeb" : "transparent")
                    }
                    onClick={() => setDrawer(p)}
                  >

...
//
// v2 — Token key fixed + stuck-loader safety net
// ─────────────────────────────────────────────────────────────
...
/*
 * ✅ FIX: was "seller_token" — must match sellerAuth.routes.js
 *    which saves the JWT as "sellerToken".
 *    Mismatch caused the loader to spin forever.
 */
...
/*
 * ✅ NEW: One-time migration for users who logged in before this fix.
 *    If they have a token under the old key, move it to the new key.
 *    This runs once on module load — before the hook mounts.
 */
(function migrateLegacyToken() {
  if (typeof window === "undefined") return;
  const NEW_KEY = "sellerToken";
  const OLD_KEYS = ["seller_token", "token", "auth_token"];

  if (localStorage.getItem(NEW_KEY)) return; /* already has correct key */

  for (const oldKey of OLD_KEYS) {
    const oldVal = localStorage.getItem(oldKey);
    if (oldVal && oldVal.split(".").length === 3) {
      /* Looks like a JWT — migrate it */
      localStorage.setItem(NEW_KEY, oldVal);
      localStorage.removeItem(oldKey);
      console.log(`[useSellerFlow] Migrated token: "${oldKey}" → "${NEW_KEY}"`);
      return;
    }
  }
})();
...
const INIT_STORE = {
  ...
  store_logo: null, store_banner: null,
  withdrawal_method: "bank_transfer",
  bank_account: "", bank_name: "", bank_code: "", account_name: "",
};

const INIT_VERIFY = {
  id_card: null, id_card_back: null, selfie: null,
  business_doc: null, address_proof: null,
...
const apiMsg    = (err, fallback) => err?.response?.data?.message ?? fallback;
...
export const useSellerFlow = () => {
  ...
  /* ═════════════════════════════════════════════════════════════
     ...
     ✅ NEW: Guaranteed to always call setInitializing(false)
             AND always set a valid step, even if everything
             throws unexpectedly.
  ═════════════════════════════════════════════════════════════ */
  useEffect(() => {
    ...
    /*
     * ✅ SAFETY NET #1: Hard timeout.
     *    If nothing has resolved after 15 s, force initializing=false.
     *    User will see the register form instead of infinite loader.
     */
    const hardTimeout = setTimeout(() => {
      console.warn("[useSellerFlow] ⚠️ Sync timeout — forcing REGISTER step");
    ...
    const syncFromServer = async () => {
      try {
        ...
        try {
          ...
          console.log("[useSellerFlow] sync response:", { status, code, msg: err.message });
          ...
          if (status === 401 || code === "INVALID_TOKEN" || code === "TOKEN_EXPIRED") {
            ...
            /* Valid token, no vendor yet — normal for new seller */
            setStepRaw(STEPS.STORE_SETUP);

          } else if (status === 0 || !err.response) {
            /*
             * ✅ FIX: Network error — don't clear token.
             *    User may just have poor connection.
             *    Show store_setup (they're logged in, can retry).
             */
            console.warn("[useSellerFlow] Network error — keeping token, showing STORE_SETUP");
            setStepRaw(STEPS.STORE_SETUP);

          } else {
            console.warn("[useSellerFlow] Unexpected sync error:", err.message);
            clearToken();
            setStepRaw(STEPS.REGISTER);
          }
        }
        ...
        /*
         * ✅ SAFETY NET #2: Any synchronous error at all —
         *    still show something instead of blank loader.
         */
        console.error("[useSellerFlow] Fatal sync error:", outerErr);
        setStepRaw(STEPS.REGISTER);
  ...
  const validateVerification = () => {
    const e = {};
    if (!verifyData.id_type) e.id_type = "Please select an ID type";
  ...
  const submitRegister = useCallback(async () => {
    ...
    try {
      ...
      setServerMsg(data.message ?? "Account created! Check your email for a 6-digit code.");
      ...
      if (code === "EMAIL_TAKEN_UNVERIFIED") {
        ...
        setPendingEmail(email);
        setStep(STEPS.OTP_VERIFY, { keepMsg: true });
        setServerMsg(err.response?.data?.message ?? "Account found! We've resent your verification code.");
        return;
      }
    ...
    try {
      ...
      if (registerData.password) {
        try {
          ...
          console.warn("[submitOtp] silent login failed:",
      ...
      const code = apiCode(err);
      if (code === "CODE_EXPIRED") setServerErr("Code has expired. Please request a new one.");
      else if (code === "INVALID_CODE") setServerErr("Incorrect code. Please check and try again.");
      else setServerErr(apiMsg(err, "Verification failed. Please try again."));
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      try {
        ...
        if (status === 404) {
          ...
          clearToken();
          setServerErr("This email belongs to a marketplace account. Please create a separate seller account.");
        } else {
      ...
      const status = httpStatus(err);
      const code   = apiCode(err);
      if (status === 401) setServerErr("Incorrect email or password.");
      else if (status === 403 && code === "EMAIL_NOT_VERIFIED") {
  ...
  /* ═════════════════════════════════════════════════════════════
     API — FORGOT PASSWORD  (unchanged)
  ═════════════════════════════════════════════════════════════ */
  const submitForgotPassword = useCallback(async (email) => {
    const cleanEmail = email?.trim().toLowerCase() ?? "";
    if (!cleanEmail) { setServerErr("Email address is required."); return; }
    if (!EMAIL_RX.test(cleanEmail)) { setServerErr("Please enter a valid email address."); return; }

    setLoading(true); setServerMsg(""); setServerErr("");
    try {
    ...
    setServerMsg("If a seller account exists with this email, a 6-digit reset code has been sent.");
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  /* ═════════════════════════════════════════════════════════════
     API — VERIFY RESET CODE  (unchanged)
  ═════════════════════════════════════════════════════════════ */
  const submitResetCode = useCallback(async (code) => {
    const trimmed = code?.trim() ?? "";
    if (!trimmed) { setServerErr("Please enter the 6-digit reset code."); return; }
    if (trimmed.length !== 6 || !/^\d{6}$/.test(trimmed)) {
      setServerErr("Code must be exactly 6 digits."); return;
    }
    setLoading(true); setServerMsg(""); setServerErr("");
    try {
      await axios.post(`${AUTH_API}/verify-reset-code`, { email: pendingEmail, code: trimmed });
      ...
      const code = apiCode(err);
      if (code === "CODE_EXPIRED") setServerErr("Reset code has expired. Please request a new one.");
  ...
  /* ═════════════════════════════════════════════════════════════
     API — SET NEW PASSWORD  (unchanged)
  ═════════════════════════════════════════════════════════════ */
  const submitNewPassword = useCallback(async (newPassword, confirmPassword) => {
    if (!newPassword) { setServerErr("New password is required."); return; }
    if (newPassword.length < 8) { setServerErr("Password must be at least 8 characters."); return; }
    if (!/[A-Z]/.test(newPassword)) { setServerErr("Password must contain at least one uppercase letter."); return; }
    if (!/\d/.test(newPassword)) { setServerErr("Password must contain at least one number."); return; }
    if (newPassword !== confirmPassword) { setServerErr("Passwords do not match."); return; }

    setLoading(true); setServerMsg(""); setServerErr("");
    try {
      await axios.post(`${AUTH_API}/reset-password`, {
        email: pendingEmail, code: verifiedResetCode, newPassword,
      });
      const resetEmail = pendingEmail;
      setPendingEmail(""); setVerifiedResetCode("");
      setStep(STEPS.REGISTER, { keepMsg: true });
      setServerMsg("✅ Password reset successfully! Please sign in with your new password.");
  ...
  /* ═════════════════════════════════════════════════════════════
     API — RESEND RESET CODE  (unchanged)
  ═════════════════════════════════════════════════════════════ */
  ...
  /* ═════════════════════════════════════════════════════════════
     API — SUBMIT VERIFICATION  (unchanged)
  ═════════════════════════════════════════════════════════════ */
  const submitVerification = useCallback(async () => {
    ...
    try {
      ...
      ["id_card", "id_card_back", "selfie", "business_doc", "address_proof"]
        .forEach((key) => { if (verifyData[key]) form.append(key, verifyData[key]); });
      ...
      setServerMsg(data.message ?? "Documents submitted! We'll review within 1–3 business days.");
    } catch (err) {
  ...
  ═════════════════════════════════════════════════════════════ */
  return {
    step, setStep, initializing, loading,
    ...
    handleVerifyChange,
    setBankField,
    submitRegister, submitOtp, resendOtp, submitLogin,
    submitForgotPassword, submitResetCode,
    submitNewPassword, resendResetCode,
    submitStore, submitVerification,
    signOut,
  };
};

 ...
 *
 * Premium marketplace layout for screens ≥ 1024px:
 * - Glassmorphism topbar
 * - Large hero with slides
 * - Multi-column featured grid
 * - Horizontal scroll sections
 * - Big product cards with all premium features
 */
...
import {
  ...
  FiCamera, FiTag, FiTruck, FiLock,
  FiRotateCcw, FiAward, FiStar, FiMapPin,
  FiPackage as FiBox,
} from "react-icons/fi";
...
function useRipple() {
  ...
  const RippleContainer = useMemo(() => (
    <>
      {ripples.map((r) => (
        <span key={r.id} className="lm-ripple"
          style={{ left: r.x, top: r.y }} aria-hidden="true" />
      ))}
    </>
  ), [ripples]);
...
const StarRating = memo(function StarRating({ rating, count }) {
  const stars = useMemo(() => {
    return Array.from({ length: 5 }, (_, i) => Math.min(1, Math.max(0, rating - i)));
  }, [rating]);
    ...
    <div className="lm-stars" aria-label={`${rating.toFixed(1)} out of 5 stars`}>
      {stars.map((fill, i) => (
        <span key={i} className="lm-star-wrap">
          <FiStar size={10} className="lm-star-bg" aria-hidden="true" />
          <FiStar size={10} className="lm-star-fg"
...
const ImageCarousel = memo(function ImageCarousel({ images, alt }) {
  ...
  return (
    <div className="lm-carousel">
      <img src={imgs[idx]} alt={`${alt} ${idx + 1}`}
        className="mp-card-img lm-card-img" loading="lazy"
        onError={(e) => { e.currentTarget.style.display = "none"; }} />
      ...
      {idx > 0 && (
        <button type="button" className="lm-carousel-arrow lm-carousel-arrow--prev"
      ...
      {idx < imgs.length - 1 && (
        <button type="button" className="lm-carousel-arrow lm-carousel-arrow--next"
...
const ProductCard = memo(function ProductCard({
  ...
  return (
    ...
    >
      <div className="mp-card-img-wrap lm-pcard-img-wrap"
        onClick={go} role="button" tabIndex={0}
        ...
        <button type="button"
          ...
          onClick={handleWish}>
          <FiHeart size={16} fill={hearted ? "currentColor" : "none"}
      ...
      <div className="mp-card-body lm-pcard-body">
        ...
        <p className="mp-card-name lm-pcard-name"
          onClick={go} style={{ cursor:"pointer" }}>
          {product.name}
        </p>

        <StarRating rating={rating} count={reviewCount} />

        {soldLabel && <p className="lm-sold-label">{soldLabel}</p>}
    ...
    <div ref={ref}
      ...
      <div className="mp-section-header lm-hscroll-header">
        <div className="mp-section-title-wrap">
          <div className="lm-section-icon-wrap"
            style={{ background: `${accent}18`, color: accent }} aria-hidden="true">
            {Icon && <Icon size={18} />}
          </div>
          <div>
...
const MiniCard = memo(function MiniCard({ product, onAddToCart, onWishlist, wishlisted }) {
  ...
  return (
    <div className="lm-mini-card">
      <div className="lm-mini-card__img-wrap"
      ...
      <div className="lm-mini-card__body"
        onClick={() => navigate(dest)} style={{ cursor:"pointer" }}>
        <p className="lm-mini-card__name">{product.name}</p>
        <StarRating rating={rating} />
        <p className="lm-mini-card__price">{fmtPrice(product.price)}</p>
        <button type="button" className="lm-mini-card__cart"
  ...
  return (
    <div className="lm-flash-card"
      ...
      <div className="lm-flash-card__img-wrap">
        ...
        {discount > 0 && <span className="lm-flash-card__discount">-{discount}%</span>}
      </div>
      <div className="lm-flash-card__body">
        ...
        <button type="button" className="lm-flash-card__cart"
          onClick={(e) => { e.stopPropagation(); onAddToCart(product); }}>
    ...
    <div className="lm-search-dropdown" role="listbox" aria-label="Search suggestions">
      {showHistory && (
        <>
          ...
          {history.map((s) => (
            <button key={s} type="button" className="lm-search-dropdown__item"
              role="option" onClick={() => onSelect(s)}>
      ...
      {showTrending && (
        <>
          ...
          <div className="lm-search-dropdown__trending">
            {trending.map((s) => (
              <button key={s} type="button" className="lm-search-dropdown__chip"
                onClick={() => onSelect(s)}>
                {s}
              </button>
            ))}
      ...
      {showResults && (
        <>
          ...
          <button type="button" className="lm-search-dropdown__item lm-search-dropdown__item--query"
            role="option" onClick={() => onSelect(query)}>
  ...
  const fetchSections = useCallback(async () => {
    try {
      ...
      if (latest.status === "fulfilled") {
        const rows = latest.value.data?.data?.products ?? [];
    ...
    <div className="mp-page lm-page lm-premium">
      ...
      <header className={`mp-topbar lm-topbar lm-glass-topbar ${scrolled ? "lm-topbar--scrolled" : ""}`}>
        <div className="mp-topbar-row">
          <button type="button" className="mp-logo lm-logo-btn"
            onClick={() => navigate("/loemart")} aria-label="Loemart home">
          ...
          <div className="mp-search-wrap lm-search-wrap" ref={searchRef}>
            <form onSubmit={handleSearch} role="search" aria-label="Search">
              <span className="mp-search-ico" aria-hidden="true"><FiSearch size={16} /></span>
              <input type="search" className="mp-search lm-search-input"
                placeholder="Search products, brands, categories…"
                value={searchQuery} aria-label="Search products" autoComplete="off"
                onChange={(e) => setSearchQuery(e.target.value)}
                onFocus={() => setSearchFocused(true)} />
              {searchQuery && (
                <button type="button" className="mp-search-clear" aria-label="Clear"
          ...
          <div className="mp-topbar-actions lm-topbar-actions">
            ...
            <button type="button" className="mp-icon-btn lm-icon-btn"
            ...
            <button type="button" className="mp-post-btn lm-post-btn" onClick={goPostAd}>
              <FiPlus size={15} />
      ...
      <section className="mp-hero lm-hero lm-hero-premium" aria-label="Hero">
        ...
        <div className="mp-hero-content lm-hero-content-premium">
          ...
          <div className="mp-hero-actions lm-hero-actions-premium">
            <button type="button" className="lm-hero-cta-primary"
          ...
          {cartCount > 0 && (
            <button type="button" className="lm-hero-cart-pill"
              onClick={() => navigate("/shop/cart")}
      ...
      {recentlyViewed.length > 0 && (
        <HScroll
          ...
          {recentlyViewed.map((p) => (
            <div key={p.id} className="lm-recent-card"
              role="button" tabIndex={0}
      ...
      <div className="mp-subbar lm-subbar lm-sticky-subbar" aria-label="Controls">
        ...
        <div className="mp-subbar-right">
          ...
          <div className="mp-sort-wrap">
            <button type="button" className="mp-sort-btn lm-sort-btn"
              aria-haspopup="listbox" aria-expanded={showSort}
              onClick={() => setShowSort((v) => !v)}>
              <FiSliders size={13} /> {activeSortLabel}
              <FiChevronRight size={13}
                style={{ transform: showSort ? "rotate(90deg)" : "none", transition:".2s" }} />
            </button>

            {showSort && (
              <>
                <div style={{ position:"fixed", inset:0, zIndex:199 }}
                  onClick={() => setShowSort(false)} aria-hidden="true" />
        ...
        <div className="mp-active-filters lm-filter-pills">
          {searchQuery && (
            <span className="mp-filter-pill lm-pill">
              ...
              <button type="button"
                onClick={() => { setSearchQuery(""); setSearchParams({}); fetchProducts({ query:"", newOffset:0 }); }}>
                <FiX size={10} />
              </button>
            </span>
          )}
          ...
          {minPrice && (
            <span className="mp-filter-pill lm-pill">
              ...
              <button type="button" onClick={() => { setMinPrice(""); fetchProducts({ min:"", newOffset:0 }); }}>
                <FiX size={10} />
              </button>
            </span>
          )}
          {maxPrice && (
            <span className="mp-filter-pill lm-pill">
              ...
              <button type="button" onClick={() => { setMaxPrice(""); fetchProducts({ max:"", newOffset:0 }); }}>
                <FiX size={10} />
              </button>
            </span>
          )}
      ...
      {/* ═══════════════════════════════════════════════
          ALL PRODUCTS GRID
      ═══════════════════════════════════════════════ */}
      <main id="loemart-listings"
        className={`mp-grid lm-grid ${view === "list" ? "mp-grid--list" : "mp-grid--grid2"}`}
        aria-label="Product listings" aria-live="polite" aria-busy={loading}>
        ...
        {!loading && !fetchError && !products.length && (
          <div className="mp-empty lm-empty-state">
            <div className="lm-empty-illustration" aria-hidden="true">
              <div className="lm-empty-circle"><FiSearch size={36} /></div>
              <div className="lm-empty-dots"><div /><div /><div /></div>
            </div>
            ...
            <div className="lm-empty-suggestions">
              {TRENDING_SEARCHES.slice(0, 4).map((s) => (
                <button key={s} type="button" className="lm-empty-suggestion"
      ...
      <FadeSection className="mp-section lm-notify-wrap" aria-label="Newsletter">
        <div className="lm-notify-card">
          <div className="lm-notify-left">
            <div className="lm-notify-icon-box" aria-hidden="true"><FiBell size={24} /></div>
            <div>
          ...
          {notifySent ? (
            ...
            <form className="lm-notify-form" onSubmit={handleNotify}>
              <input type="email" className="lm-notify-input"
                placeholder="your@email.com" value={notifyEmail}
      ...
      ═══════════════════════════════════════════════ */}
      {cartCount > 0 && (
        <button type="button" className="lm-float-cart"
          onClick={() => navigate("/shop/cart")}
          aria-label={`Cart — ${cartCount} items`}>
          <FiShoppingCart size={19} />
        ...
        <>
          ...
          <div className="mp-drawer lm-drawer" role="dialog" aria-modal="true" aria-label="Filters">
            ...
            <div className="mp-drawer-header">
              ...
              <button type="button" className="mp-drawer-close"
                onClick={() => setShowFilters(false)} aria-label="Close">
                <FiX size={15} />
              </button>
            </div>
              ...
              <div className="mp-price-range">
                <div className="mp-price-input-wrap">
                  ...
                  <input type="number" className="mp-price-input" placeholder="Min"
                    value={minPrice} min={0} aria-label="Min price"
                ...
                <div className="mp-price-input-wrap">
                  ...
                  <input type="number" className="mp-price-input" placeholder="Max"
                    value={maxPrice} min={0} aria-label="Max price"
            ...
            <div className="mp-drawer-footer">
              <button type="button" className="mp-btn-clear"
                onClick={() => {
                  ...
                  setActiveSort("newest"); setActiveCategory("all");
                }}>
                Reset All
              </button>

    ...
    <>
      ...
      <div className="lmm-sheet" role="dialog" aria-modal="true" aria-label="Filters">
            ...
            <div className="lmm-sheet__chips">
              {SORT_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  type="button"
                  className={`lmm-sheet__chip ${
                    activeSort === opt.value ? "lmm-sheet__chip--on" : ""
                  }`}
        ...
        <div className="lmm-sheet__footer">
          <button type="button" className="lmm-btn-reset" onClick={onReset}>
            Reset
          </button>
          <button type="button" className="lmm-btn-apply" onClick={onApply}>
            Apply Filters
          </button>
        </div>
      </div>
...
export function CartToast({ product, onView, onClose }) {
  return (
    <div className="lmm-toast" role="status" aria-live="polite">
      <div className="lmm-toast__img-wrap">
        {primaryImg(product?.images) ? (
          ...
          <div className="lmm-toast__ph">
            <FiPackage size={16} />
          </div>
        )}
      </div>
      <div className="lmm-toast__body">
        <p className="lmm-toast__label">✓ Added to Cart</p>
        <p className="lmm-toast__name">{product?.name || "Item"}</p>
      </div>
      <button type="button" className="lmm-toast__view" onClick={onView}>
        View
      </button>
      <button
        type="button"

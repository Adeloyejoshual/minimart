...
import { FiArrowRight, FiZap, FiStar, FiTrendingUp, FiPlusCircle, FiTag, FiShoppingBag } from "react-icons/fi";
...
const MobileHero = memo(function MobileHero({ user, onPostAd, slides = DEFAULT_SLIDES }) {
  ...
  const nextSlide = useCallback(() => setSlideIndex((p) => (p + 1) % slides.length), [slides.length]);
  const prevSlide = useCallback(() => setSlideIndex((p) => (p - 1 + slides.length) % slides.length), [slides.length]);
  ...
  const handleTouchStart = (e) => { setIsPaused(true); touchStartX.current = e.touches[0].clientX; };
  const handleTouchEnd = (e) => {
    setIsPaused(false);
    if (!touchStartX.current) return;
    const diff = touchStartX.current - e.changedTouches[0].clientX;
    if (Math.abs(diff) > 40) { diff > 0 ? nextSlide() : prevSlide(); haptic(8); }
  ...
  return (
    <div className="lmm-hero-container">
      ...
      {user && firstName && (
        <div className="lmm-welcome-pill" onClick={() => navigate("/account/profile")}>
          <div className="lmm-welcome-avatar">{firstName.charAt(0).toUpperCase()}</div>
          <span className="lmm-welcome-text">Hi, <strong>{firstName}</strong></span>
      ...
      >
        <div className="lmm-hero-content">
          ...
          <button className="lmm-hero-cta" onClick={() => handleSlideAction(currentSlide.target)}>
            {currentSlide.cta}
            <div className="lmm-hero-cta-icon"><FiArrowRight size={14} strokeWidth={3} /></div>
          </button>
        </div>
        ...
        <div className="lmm-hero-pagination">
          {slides.map((_, idx) => (
            <div key={idx} className={`lmm-hero-dot ${idx === slideIndex ? "active" : ""}`} />
          ))}
        </div>
      </section>

      {/* Quick Action Tiles */}
      <nav className="lmm-quick-actions">
        {quickTiles.map((tile) => (
          <button 
            key={tile.id} 
            className="lmm-quick-tile" 
            onClick={() => { haptic(6); tile.action ? tile.action() : document.querySelector(".lmm-catalog-grid-segment")?.scrollIntoView({ behavior: "smooth" }); }}
          >
            <div className="lmm-quick-tile__icon" style={{ backgroundColor: tile.bg, color: tile.color }}>

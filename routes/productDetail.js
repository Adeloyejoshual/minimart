...
const normalizeProduct = (row, productImageRows = []) => {
  ...
  // Normalize specifications to array of { label, value }
  let specsArray = [];
  ...
  return {
    ...
    // ✅ Seller — all from users JOIN, no duplicate
    ...
    seller_trust    : row.seller_trust    ? Number(row.seller_trust)  : null,
...
/* ═══════════════════════════════════════════════════════════════
   COLUMNS
   ✅ FIX: Removed duplicate p.seller_name
═══════════════════════════════════════════════════════════════ */
const PRODUCT_COLS = `
  p.id,
  p.slug,
...
/* ═══════════════════════════════════════════════════════════════
   GET /slug/:slug
   ✅ Added detailed error logging to pinpoint exact failure
═══════════════════════════════════════════════════════════════ */
router.get("/slug/:slug", async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      // ✅ Log exactly why it's 404 for debugging
      const debugRow = await pool.query(
        `SELECT id, slug, status, is_active FROM public.products WHERE slug = $1 LIMIT 1`,
        [slug]
      );
      if (!debugRow.rows.length) {
        console.warn(`[slug/${slug}] → No product with this slug exists`);
      } else {
        const d = debugRow.rows[0];
        console.warn(
          `[slug/${slug}] → Found but filtered out:`,
          `status=${d.status}`,
    ...
    console.error(err.stack); // ✅ Full stack for DB errors
...
router.get("/id/:id", async (req, res) => {
  ...
  try {
    ...
    console.error(`[productDetail] GET /id/${req.params.id} →`, err.message);
    return res.status(500).json({ message: "Failed to load product" });
  }
});
...
router.get("/slug/:slug/reviews", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("[productDetail] reviews table missing or error:", e.message);
    }
...
router.get("/users/:userId/favorites", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT
         p.id, p.slug, p.title, p.price, p.original_price,
         p.condition, p.main_image, p.thumbnail_url, p.images,
         p.location_city, p.location_state, p.is_promoted,
         p.boost_score, p.status, p.active_until, p.created_at,
...
router.get("/users/:id/public", async (req, res) => {
  ...
  try {
    const [userResult, listingsResult] = await Promise.all([
      pool.query(
        `SELECT
           id, name, store_name, store_description, store_logo,
           profile_image, store_verified, verified, identity_verified,
           trust_score, rating, products_count, total_sales, is_online,
           created_at,

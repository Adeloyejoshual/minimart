 ...
 * POST /api/auth/reset-password           — set new password (uses reset_token)
 *
 * Changes aligned with AuthPage.jsx:
 *   [1] normalizeEmail()       — trim + lowercase on every email input
 *   [2] EMAIL_RE               — stricter regex matching frontend (/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/)
 *   [3] Password strength      — uppercase + number validated on register too
 *   [4] Forgot-password        — safe response always 200 (never reveals if email exists)
 *   [5] OTP_LOCKED             — returns attemptsLeft: 0 + code so frontend ForgotPanel can react
 *   [6] reset-password         — validates symbol too (matches frontend strength score >= 2)
 */
...
/* [1] Always trim + lowercase — mirrors AuthPage normalizeEmail() */
const normalizeEmail = (raw = "") => raw.trim().toLowerCase();

/* [2] Stricter regex — matches frontend EMAIL_RE exactly */
...
/* ═══════════════════════════════════════════════════════════════
   AUTO-MIGRATION
   Creates password_reset_otps on first startup if not exists.
═══════════════════════════════════════════════════════════════ */
...
const authLimiter   = mkLimiter({ windowMin: 15, max: 10,
  message: "Too many attempts. Please try again later."           });
const forgotLimiter = mkLimiter({ windowMin: 60, max: 5,
  message: "Too many reset requests. Please try again in an hour." });
const verifyLimiter = mkLimiter({ windowMin: 15, max: 10,
  message: "Too many verification attempts. Please try again later." });
const resetLimiter  = mkLimiter({ windowMin: 15, max: 10,
...
router.post("/register", authLimiter, async (req, res, next) => {
  ...
  /* [1][2] Normalize + validate email */
  const cleanEmail = normalizeEmail(email);
  if (!isValidEmail(cleanEmail))
    return fail(res, 400, "Please enter a valid email address.");

  /* ── Password strength — [3] matches frontend getStrength() ── */
...
router.post("/login", authLimiter, async (req, res, next) => {
  ...
  try {
    ...
    /* [4] Generic message — never reveal whether email exists */
    if (!rows.length)
    ...
    /* fire-and-forget last_login + online update */
    pool.query(
...
/* ═══════════════════════════════════════════════════════════════
   ...
   [4] Always returns 200 — never reveals if email exists.
═══════════════════════════════════════════════════════════════ */
router.post("/forgot-password", forgotLimiter, async (req, res, next) => {
  ...
  /* [4] Safe generic response — same whether email exists or not */
  const SAFE_RESPONSE = {
    success : true,
  ...
  try {
    ...
    /* no account — return safe response without revealing this */
    if (!rows.length) {
    ...
    /* generate OTP — hash it before storing */
    ...
    try {
      ...
      if (!IS_PROD) {
        /*
         * Dev only — email failed but still allow flow using console OTP.
         * dev_otp is shown in AuthPage's yellow dev banner.
         */
        return res.json({
          ...SAFE_RESPONSE,
          dev_otp  : rawOtp,
      ...
      /* Prod — invalidate OTP and surface error */
      await pool.query(
...
/* ═══════════════════════════════════════════════════════════════
   ...
   [5] Returns reset_token + attemptsLeft for AuthPage ForgotPanel.
═══════════════════════════════════════════════════════════════ */
router.post("/forgot-password/verify", verifyLimiter, async (req, res, next) => {
  ...
  try {
    ...
    if (!otpRows.length) {
      ...
      /* [5] Lock OTP when max attempts reached */
      if (attemptsLeft === 0) {
        await pool.query(
          `UPDATE password_reset_otps
           SET    used = true
      ...
      /* [5] Return remaining attempts so AuthPage can show the counter */
      return fail(res, 400,
    ...
    /*
     * Issue a short-lived reset_token.
     * AuthPage ForgotPanel sends this in step 3.
     * Expires in 15 min (same as OTP).
     */
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Step 3 — set new password using reset_token from step 2.
   [6] Password rules match AuthPage getStrength() — score >= 2
       requires uppercase + number at minimum.
═══════════════════════════════════════════════════════════════ */
router.post("/reset-password", resetLimiter, async (req, res, next) => {
  ...
  /* [6] Password strength — mirrors AuthPage strength.score >= 2 (Fair) */
  if (password.length < 8)
    return fail(res, 400, "Password must be at least 8 characters.");
  if (!/[A-Z]/.test(password))
    return fail(res, 400, "Password must contain at least one uppercase letter.");
  ...
  try {
    ...
    /* confirm the OTP record is still verified and not yet used */
    ...
    /* mark this OTP as used — one-time only */
    await client.query(
      `UPDATE password_reset_otps

 ...
 *
 * v5 changes:
 * ✓ Role-based transition guard via isTransitionAllowed()
 * ✓ Writes order_status_history on every status change
 * ✓ Calls recomputeGroupStatus() to update parent order
 * ✓ Fires sendShipmentNotifications() on "shipped"
 * ✓ Updates seller_earnings on delivered/cancelled
 * ✓ Sets shipped_at / cancelled_at timestamps
 * ✓ POST /ready — seller marks items ready for Loemart Express pickup
 * ✓ GET /:orderId includes dispatch, history, earnings
 * ✓ GET / includes sub-order tracking_id + dispatch info
 * ✓ Stats include earnings summary
 */
...
/* ══════════════════════════════════════════════════════════════
   ...
   Returns sub-order tracking_id, parent tracking_id, dispatch info.
══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   ...
   Restricted to: confirmed, processing, shipped, cancelled.
   Everything after "shipped" is admin/system/buyer territory.
══════════════════════════════════════════════════════════════ */
router.patch("/:orderId/status", async (req, res) => {
  ...
  try {
     ...
     * Also fetch buyer info for notifications.
     */
    ...
    const newGroupStatus = await recomputeGroupStatus(
      client,
      order.order_group_id
    );
    ...
    if (newStatus === "shipped") {
      /*
       * "shipped" gets the full shipment notification suite:
       * buyer email, seller email, dispatch alert, in-app for both.
       */
      ...
      /*
       * Other transitions get basic status update notifications.
       */
...
/* ══════════════════════════════════════════════════════════════
   POST /:orderId/ready
   ─────────────────────────────────────────────────────────────
   Seller marks items as "packed and ready for Loemart Express
   pickup". This does NOT change the status — it sets the
   pickup_ready_at timestamp and creates an order_dispatches
   row so the dispatch team knows to send an agent.

   Valid when status is "confirmed" or "processing".
══════════════════════════════════════════════════════════════ */

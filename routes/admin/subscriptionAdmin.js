...
const applyPlanChange = async (client, userId, planSlug, billingCycle = null) => {
  ...
  if (planSlug === "free") {
    ...
    await client.query(
      `UPDATE users SET
         ...
         billing_cycle = NULL, subscription_started_at = NULL,
         subscription_expires_at = NULL, auto_renew = TRUE, updated_at = NOW()
       WHERE id = $1`, [userId]);
    await client.query(
      `UPDATE listings SET search_priority = 0, updated_at = NOW() WHERE user_id = $1`,
      [userId]);
    return { plan };
  }
  ...
  await client.query(
    `UPDATE users SET
       subscription_plan = $1, subscription_status = 'active',
       billing_cycle = $2, subscription_started_at = NOW(),
       subscription_expires_at = $3, auto_renew = TRUE, updated_at = NOW()
     WHERE id = $4`,
    [plan.slug, cycle, expiresAt, userId]
  );

  await client.query(
    `UPDATE listings SET search_priority = $1, updated_at = NOW() WHERE user_id = $2`,
    [plan.rank, userId]
  );
...
/* ═══════════════════════════════════════════════════════════════════════════
   NAMED ROUTES — must come before /:userId routes
═══════════════════════════════════════════════════════════════════════════ */
  ...
  try {
    ...
    const { rows: subscriptions } = await client.query(
      `SELECT
         ...
         s.amount, s.currency, s.payment_reference,
         s.status, s.auto_renew, s.started_at, s.expires_at, s.created_at,
         u.name                                     AS user_name,
...
router.get("/stats", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const byPlan = planRes.rows.reduce((acc, r) => { acc[r.plan_slug] = safeInt(r.count); return acc; }, {});
    ...
    const [dailyRes, weeklyRes, monthlyRes] = await Promise.all([
      safe(client.query(
        `SELECT TO_CHAR(DATE(paid_at), 'MM-DD') AS label,
         ...
         WHERE status = 'success' AND paid_at >= NOW() - INTERVAL '30 days'
      ...
      safe(client.query(
        `SELECT TO_CHAR(DATE_TRUNC('week', paid_at), 'MM-DD') AS label,
                COALESCE(SUM(amount)/100, 0)                  AS amount
         FROM payment_transactions
         WHERE status = 'success' AND paid_at >= NOW() - INTERVAL '12 weeks'
         GROUP BY DATE_TRUNC('week', paid_at) ORDER BY DATE_TRUNC('week', paid_at)`
      )),
      safe(client.query(
...
router.get("/expiring", verifyAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await client.query(
      `SELECT s.id, s.user_id, s.plan_slug, s.billing_cycle,
  ...
  try {
    const { rows } = await client.query(
      `SELECT
         ...
         COUNT(*) FILTER (WHERE status='cancelled' AND updated_at>=NOW()-INTERVAL '7 days') AS cancelled_week,
         COUNT(*) FILTER (WHERE status='expired')    AS expired,
         COUNT(*) FILTER (WHERE status='active' AND metadata::text ILIKE '%reactivate%') AS reactivated,
         COUNT(*) FILTER (WHERE status='active' AND renewal_type='auto') AS renewed
       FROM subscriptions`
    );
    const r = rows[0] ?? {};
    res.json({
...
router.get("/forecast", verifyAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await client.query(
      `SELECT
         COUNT(*) FILTER (WHERE expires_at BETWEEN NOW() AND NOW()+INTERVAL '7 days')  AS next_7_count,
         COUNT(*) FILTER (WHERE expires_at BETWEEN NOW() AND NOW()+INTERVAL '30 days') AS next_30_count,
         COALESCE(SUM(amount) FILTER (WHERE expires_at BETWEEN NOW() AND NOW()+INTERVAL '7 days'),0)  AS revenue_7,
         COALESCE(SUM(amount) FILTER (WHERE expires_at BETWEEN NOW() AND NOW()+INTERVAL '30 days'),0) AS revenue_30
       FROM subscriptions WHERE status='active' AND auto_renew=TRUE`
    );
    const r = rows[0] ?? {};
    res.json({
...
router.get("/top-subscribers", verifyAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await client.query(
      `SELECT s.user_id, u.name AS user_name, u.email AS user_email,
              s.plan_slug, SUM(s.amount) AS total_spend
       FROM subscriptions s LEFT JOIN users u ON u.id = s.user_id
       WHERE s.status != 'failed'
       GROUP BY s.user_id, u.name, u.email, s.plan_slug
       ORDER BY total_spend DESC LIMIT 10`
    );
    res.json({ subscribers: rows.map((r) => ({ ...r, total_spend: safeNum(r.total_spend) })) });
  } catch (err) {
...
router.get("/audit-log", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT al.id, al.admin_id, al.admin_name, al.action,
              al.target_user_id, al.detail, al.created_at,
              u.name AS target_user, u.email AS target_email
...
router.get("/export", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (status && status !== "all") { conditions.push(`s.status = $${idx++}`);    values.push(status); }
    if (q?.trim()) {
      conditions.push(`(LOWER(CAST(u.name AS TEXT)) LIKE $${idx} OR LOWER(CAST(u.email AS TEXT)) LIKE $${idx})`);
      values.push(`%${q.trim().toLowerCase()}%`);
      idx++;
    }
    if (ids) {
      const idArray = ids.split(",").filter(Boolean);
      if (idArray.length) { conditions.push(`s.id = ANY($${idx++}::uuid[])`); values.push(idArray); }
    }
    ...
    const { rows } = await client.query(
      `SELECT s.id, u.name, u.email,
              ...
              s.plan_slug, s.billing_cycle,
              s.amount / 100 AS amount_naira,
              s.status, s.auto_renew,
              s.started_at, s.expires_at, s.payment_reference, s.created_at
       FROM subscriptions s LEFT JOIN users u ON u.id = s.user_id
       ${where} ORDER BY s.created_at DESC LIMIT 5000`,
      values
    );

    if (format === "csv") {
      const header = "ID,Name,Email,Phone,Plan,Cycle,Amount (₦),Status,Auto Renew,Started,Expires,Reference,Created";
...
// Dedicated user search for AssignPlanModal
...
router.get("/search-users", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    // Use LOWER(CAST(col AS TEXT)) LIKE for CockroachDB STRING compatibility
    const { rows } = await client.query(
      `SELECT
         id,
         name,
...
router.post("/verify-payment", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(
      ...
      { headers: { Authorization: `Bearer ${process.env.PAYSTACK_SECRET_KEY}` }, timeout: 12000 }
    );
...
router.post("/bulk/:action", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (action === "extend") {
      await client.query(
        `UPDATE subscriptions SET expires_at=expires_at+INTERVAL '30 days', updated_at=NOW()
...
/* ═══════════════════════════════════════════════════════════════════════════
   :userId PARAM ROUTES — must come AFTER all named routes above
═══════════════════════════════════════════════════════════════════════════ */
...
router.get("/:userId/payments", verifyAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await client.query(
      `SELECT pt.id, pt.reference, pt.provider,
              pt.amount, pt.amount / 100 AS amount_naira,
              pt.currency, pt.status, pt.type, pt.paid_at, pt.created_at,
              s.plan_slug, sp.name AS plan_name
...
router.get("/:userId/features", verifyAdmin, async (req, res) => {
  ...
  try {
    const { rows: userRows } = await client.query(
      `SELECT subscription_plan FROM users WHERE id = $1`, [req.params.userId]);
    ...
    const { rows: featureRows } = await client.query(
      `SELECT sf.feature_key, sf.feature_value
       ...
       WHERE sp.slug = $1`, [planSlug]);

    const features = featureRows.reduce((acc, r) => { acc[r.feature_key] = r.feature_value; return acc; }, {});

    const { rows: overrideRows } = await client.query(
      `SELECT feature_key, feature_value FROM subscription_feature_overrides WHERE user_id = $1`,
      [req.params.userId]
    ).catch(() => ({ rows: [] }));

    const overrides = overrideRows.reduce((acc, r) => { acc[r.feature_key] = r.feature_value; return acc; }, {});
  ...
  try {
    const { rows } = await client.query(
      `SELECT id, content, admin_name, created_at
       FROM subscription_notes WHERE user_id = $1
...
router.post("/:userId/notes", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const adminName = req.admin?.name ?? "Admin";
    await client.query(
      `INSERT INTO subscription_notes (user_id, content, admin_id, admin_name, created_at)
       VALUES ($1,$2,$3,$4,NOW())`,
...
router.post("/:userId/change-plan", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    await logAction(client, req.admin.id, adminName, `Changed plan to ${plan}`, userId, "Admin change — no charge.");
    await client.query("COMMIT");
...
router.post("/:userId/grant", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const { rows: planRows } = await client.query(
      `SELECT id, slug, name, rank FROM subscription_plans WHERE slug = $1`, [plan]);
    ...
    await client.query(
      `UPDATE users SET
         subscription_plan=$1, subscription_status='active',
         billing_cycle=$2, subscription_started_at=NOW(),
         subscription_expires_at=$3, auto_renew=FALSE, updated_at=NOW()
       WHERE id=$4`,
...
router.post("/:userId/extend", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (days) {
      await client.query(
        `UPDATE subscriptions SET expires_at=expires_at+($1||' days')::INTERVAL, updated_at=NOW()
         WHERE user_id=$2 AND status='active'`, [safeInt(days, 30), userId]);
      await client.query(
        `UPDATE users SET subscription_expires_at=subscription_expires_at+($1||' days')::INTERVAL, updated_at=NOW()
         WHERE id=$2`, [safeInt(days, 30), userId]);
    } else {
      await client.query(
        `UPDATE subscriptions SET expires_at=$1, updated_at=NOW()
         WHERE user_id=$2 AND status='active'`, [new Date(until_date), userId]);
      await client.query(
  ...
  try {
    ...
    await client.query(
      `UPDATE subscriptions SET status='cancelled', auto_renew=FALSE, updated_at=NOW()
       WHERE user_id=$1 AND status='active'`, [userId]);
    await client.query(
      `UPDATE users SET auto_renew=FALSE, updated_at=NOW() WHERE id=$1`, [userId]);
  ...
  try {
    ...
    await client.query(
      `UPDATE subscriptions SET status='suspended', updated_at=NOW()
       WHERE user_id=$1 AND status='active'`, [userId]);
    await client.query(
      `UPDATE users SET subscription_status='suspended', updated_at=NOW() WHERE id=$1`, [userId]);
  ...
  try {
    ...
    const { rows } = await client.query(
      `UPDATE subscriptions
       ...
       WHERE user_id=$2 AND status IN ('cancelled','expired','suspended')
    ...
    await logAction(client, req.admin.id, adminName, "Reactivated subscription for 30 days", userId);
    await client.query("COMMIT");
  ...
  try {
    ...
    await client.query(
      `UPDATE subscriptions SET auto_renew=$1, updated_at=NOW()
       WHERE user_id=$2 AND status='active'`, [autoRenew, userId]);
    await client.query(
      `UPDATE users SET auto_renew=$1, updated_at=NOW() WHERE id=$2`, [autoRenew, userId]);
    await logAction(
      client, req.admin.id, adminName,
...
router.post("/:userId/refund", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE payment_transactions SET status='refunded', updated_at=NOW() WHERE id=$1`, [tx.id]);
...
router.post("/:userId/feature-override", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    await logAction(client, req.admin.id, adminName, `Override: ${key} = ${value}`, userId);
...
router.post("/:userId/notify", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    // const { rows: [user] } = await client.query(`SELECT email, name FROM users WHERE id=$1`, [userId]);
    // await sendEmail({ to: user.email, template: type, name: user.name });
    // ──────────────────────────────────────────────────────────────────────

    await logAction(client, req.admin.id, adminName, `Sent notification: ${type}`, userId);

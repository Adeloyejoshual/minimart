// ════════════════════════════════════════════════════════════════
// FILE: routes/admin/verification.js — v7
// ════════════════════════════════════════════════════════════════
...
/* ══════════════════════════════════════════════════════════════
   ...
   Caller MUST hold FOR UPDATE on the users row.
══════════════════════════════════════════════════════════════ */
...
const getNotes = async (db, verificationId, verificationType) => {
  const { rows } = await db.query(
    `SELECT vn.id, vn.action, vn.note, vn.created_at,
            u.name AS admin_name, u.email AS admin_email
     FROM   verification_notes vn
     JOIN   public.users u ON u.id = vn.admin_id
     WHERE  vn.verification_id = $1 AND vn.verification_type = $2
...
const getTimeline = async (db, verificationId, verificationType) => {
  const { rows } = await db.query(
    `SELECT vn.action AS type, vn.note, vn.created_at, u.name AS admin_name
     FROM   verification_notes vn
     JOIN   public.users u ON u.id = vn.admin_id
     WHERE  vn.verification_id = $1 AND vn.verification_type = $2
     ORDER  BY vn.created_at ASC`,
...
const downgradeListings = (client, userId) =>
  client.query(
    `UPDATE products SET status='active_limited', updated_at=NOW()
...
const buildDuplicateWarnings = async (db, userId, docHash) => {
  ...
  const { rows } = await db.query(
    `SELECT user_id FROM identity_verifications
     WHERE  document_number_hash=$1 AND user_id<>$2 AND status='approved'
     LIMIT  5`,
    [docHash, userId]
  );
  ...
  return [{
    ...
    severity          : "critical",
    detail            :
      `Document already approved on ${rows.length} other account(s).`,
...
/* ══════════════════════════════════════════════════════════════
   AUDIT
══════════════════════════════════════════════════════════════ */
const log = ({ adminId, action, targetId, details, meta=null, userId=null, ip=null }) =>
  Promise.all([
    pool.query(
    ...
    writeAudit({
      ...
      targetType: "verification", targetId,
...
const generateSignedUrl = (verificationId, field, adminId) => {
  if (!verificationId || !field || !adminId)
    throw new Error("generateSignedUrl: all params required");
...
const verifySignedToken = (token, verificationId, field, adminId) => {
  ...
  } catch { return "Invalid token encoding."; }
  ...
  if (!_bulkLimiter) {
    try {
      ...
      _bulkLimiter = rateLimit({
        ...
        message: { success: false, error: "Too many bulk operations. Wait 60s." },
      });
    } catch { _bulkLimiter = (_q,_r,n) => n(); }
  }
...
const notifyApproved = ({ userId, email, name }) =>
  Promise.all([
    ...
    createNotification({
      ...
      message: "Your identity and store have been verified. Listings are now permanent.",
    }),
...
const notifyReset = ({ userId, email, name, note }) =>
  Promise.all([
    ...
    createNotification({
      userId, type: "verification_reset",
...
const notifyIdentityApproved = ({ userId, email, name }) =>
  Promise.all([
    ...
    createNotification({
      userId, type: "identity_approved",
      title  : "Identity Verified ✓",
      message: "Your identity has been verified. Listings are now permanent.",
    }),
...
const notifyStoreRejected = ({ userId, email, name, reason }) =>
  Promise.all([
    ...
    createNotification({
      userId, type: "store_rejected",
      title  : "Store Verification Update",
      message: `Your store was not approved. Reason: ${reason}`,
    }),
...
/* ══════════════════════════════════════════════════════════════
   ...
   1. /stats
   2. GET  /identity
   3. POST /identity/bulk-*   ← BEFORE /:id
   4. GET|POST /identity/:id/*
   5. GET  /store
   6. GET|POST /store/:id/*
   7. POST /email/:userId/*
   8. POST /trust/:userId/*
   9. POST /:userId/*          ← catch-all, LAST
══════════════════════════════════════════════════════════════ */
  ...
  try {
    const [idQ, stQ, usQ, liQ, noQ] = await Promise.all([
      ...
      pool.query(`
        ...
        FROM products WHERE status='active_limited'`),
      pool.query(`
        SELECT COUNT(*)::INT AS total FROM verification_notes
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT
         ...
         iv.face_match, iv.face_confidence,
         u.id            AS user_id,
         u.name          AS user_name,
         u.email         AS user_email,
         u.phone_number  AS user_phone,
         u.status        AS user_status,
         u.trust_score, u.identity_verified, u.email_verified,
    ...
    const { rows: cr } = await pool.query(
      `SELECT COUNT(*)::INT AS total FROM identity_verifications iv ${where}`,
      params
    );

    return res.json({
      success: true, verifications: rows,
      total: cr[0].total, limit, offset,
    });

  } catch (err) {
    if (err.code === "42703") {
      console.error("[GET /identity] Missing column — run migration\n", err.message);
      return fail(res, 503,
...
/* ──────────────────────────────────────────────────────────────
   3a. POST /identity/bulk-approve  ← BEFORE /:id
────────────────────────────────────────────────────────────── */
router.post("/identity/bulk-approve", bulkRateLimit, async (req, res) => {
  ...
  await runParallel(ids, async (id) => {
    ...
    try {
      ...
      const rec = rows[0];
      await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [rec.user_id]);
      await client.query(
      ...
      await addNote(client, {
        verificationId: id, verificationType: "identity",
  ...
  log({
    ...
    details: `Bulk approved ${results.approved.length}/${ids.length}`,
    meta: results, ip,
  });
...
/* ──────────────────────────────────────────────────────────────
   3b. POST /identity/bulk-reject  ← BEFORE /:id
────────────────────────────────────────────────────────────── */
router.post("/identity/bulk-reject", bulkRateLimit, async (req, res) => {
  ...
  await runParallel(ids, async (id) => {
    ...
    try {
      ...
      await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [user_id]);
      await client.query(
        `UPDATE identity_verifications
  ...
  log({
    ...
    details: `Bulk rejected ${results.rejected.length}/${ids.length}`,
    meta: results, ip,
  });
  return res.json({ success: true, results });
});
...
router.get("/identity/:id", async (req, res) => {
  try {
    const [verRes, notes, timeline] = await Promise.all([
      pool.query(
        `SELECT
           ...
           u.identity_verified, u.email_verified, u.store_verified,
    ...
    const duplicateWarnings = rec.duplicate_warnings?.length
      ? rec.duplicate_warnings
      : await buildDuplicateWarnings(pool, rec.user_id, rec.document_number_hash);
...
/* ──────────────────────────────────────────────────────────────
   4b. GET /identity/:id/files/:field
────────────────────────────────────────────────────────────── */
router.get("/identity/:id/files/:field", async (req, res) => {
  ...
  const err = verifySignedToken(req.query.token, id, field, req.admin.id);
  if (err) return fail(res, 401, err);
  try {
    const colMap = { front:"front_image_url", back:"back_image_url", selfie:"selfie_url" };
    const { rows } = await pool.query(
      `SELECT ${colMap[field]} AS url FROM identity_verifications WHERE id=$1`, [id]
    );
    if (!rows.length || !rows[0].url) return fail(res, 404, "Document not found.");
    return res.redirect(302, rows[0].url);
  } catch (err) { return fail(res, 500, err.message); }
});

/* ──────────────────────────────────────────────────────────────
   4c. POST /identity/:id/approve
────────────────────────────────────────────────────────────── */
router.post("/identity/:id/approve", async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) { await client.query("ROLLBACK"); return fail(res, 404, "Identity verification not found."); }
    const rec = rows[0];
    if (rec.current_status === "approved") { await client.query("ROLLBACK"); return res.json({ success: true, message: "Already approved." }); }

    if (rec.document_number_hash) {
      const { rows: dup } = await client.query(
        `SELECT id FROM identity_verifications
         WHERE document_number_hash=$1 AND user_id<>$2 AND status='approved' LIMIT 3`,
        ...
        await client.query(
          `UPDATE identity_verifications
           SET status='flagged', flagged_for_review=TRUE,
               risk_score=GREATEST(COALESCE(risk_score,0),90), updated_at=NOW()
           WHERE id=$1`,
          [req.params.id]
        );
        await addNote(client, { verificationId: req.params.id, verificationType: "identity", adminId, action: "flagged", note: `Blocked: document matches ${dup.length} approved account(s).` });
        await client.query("COMMIT");
    ...
    const trustScore = await refreshAndPersistTrust(client, rec.user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "identity", adminId, action: "approved", note });
    await client.query("COMMIT");

    reactivateLimitedListings(rec.user_id).catch(() => {});
    notifyIdentityApproved({ userId: rec.user_id, email: rec.email, name: rec.name });
    log({ adminId, action: "approve_identity", targetId: req.params.id, details: `Approved identity for user ${rec.user_id}`, meta: { trust_score: trustScore, note }, userId: rec.user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Identity approved." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/identity/:id/reject", async (req, res) => {
  ...
  const reason  = (req.body.reason ?? "").trim();
  if (!reason)                         return fail(res, 400, "Rejection reason is required.");
  if (reason.length > REASON_MAX_LEN)  return fail(res, 400, `Reason must be ≤ ${REASON_MAX_LEN} characters.`);
  ...
  try {
    ...
    if (current_status === "rejected") { await client.query("ROLLBACK"); return res.json({ success: true, message: "Already rejected." }); }
    ...
    await client.query(`UPDATE public.users SET identity_verified=FALSE, updated_at=NOW() WHERE id=$1`, [user_id]);
    await downgradeListings(client, user_id);
    const trustScore = await refreshAndPersistTrust(client, user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "identity", adminId, action: "rejected", note: reason });
    await client.query("COMMIT");

    notifyIdentityRejected({ userId: user_id, email, name, reason });
    log({ adminId, action: "reject_identity", targetId: req.params.id, details: `Rejected identity for user ${user_id}`, meta: { reason, trust_score: trustScore }, userId: user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Identity rejected." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/identity/:id/reset", async (req, res) => {
  ...
  try {
    ...
    await client.query(`UPDATE public.users SET identity_verified=FALSE, updated_at=NOW() WHERE id=$1`, [user_id]);
    const trustScore = await refreshAndPersistTrust(client, user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "identity", adminId, action: "reset", note });
    await client.query("COMMIT");

    notifyReset({ userId: user_id, email, name, note });
    log({ adminId, action: "reset_identity", targetId: req.params.id, details: `Reset identity for user ${user_id}`, meta: { note, trust_score: trustScore }, userId: user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Identity reset." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/identity/:id/assign", async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return fail(res, 404, "Verification not found or not pending.");
    log({ adminId, action: "assign_identity", targetId: req.params.id, details: `Assigned to admin ${assignedAdminId}`, meta: { assigned_to: assignedAdminId }, userId: rows[0].user_id, ip });
...
router.post("/identity/:id/note", async (req, res) => {
  ...
  if (note.length > NOTE_MAX_LEN)  return fail(res, 400, `Note must be ≤ ${NOTE_MAX_LEN} characters.`);
  try {
    const { rows } = await pool.query("SELECT id FROM identity_verifications WHERE id=$1", [req.params.id]);
    if (!rows.length) return fail(res, 404, "Verification not found.");
    await addNote(pool, { verificationId: req.params.id, verificationType: "identity", adminId, action: "note", note });
...
router.get("/store", async (req, res) => {
  ...
  try {
    ...
    if (rawStatus !== "all") { params.push(rawStatus); conditions.push(`sv.status=$${params.length}`); }
    const where      = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
    const listParams = [...params, limit, offset];

    const { rows } = await pool.query(
      `SELECT sv.id, sv.documents_url, sv.status,
              ...
              u.phone_number AS user_phone, u.status AS user_status,
              u.trust_score, u.store_verified, u.identity_verified, u.email_verified
    ...
    const { rows: cr } = await pool.query(
      `SELECT COUNT(*)::INT AS total FROM store_verifications sv ${where}`, params
    );
    return res.json({ success: true, verifications: rows, total: cr[0].total, limit, offset });
  } catch (err) {
  ...
  try {
    const [verRes, notes, timeline] = await Promise.all([
      pool.query(
        `SELECT sv.id, sv.documents_url, sv.status,
                sv.rejection_reason, sv.reviewed_by, sv.reviewed_at,
                sv.assigned_admin_id, sv.assigned_at, sv.created_at, sv.updated_at,
                u.id AS user_id, u.name AS user_name, u.email AS user_email,
                u.phone_number AS user_phone, u.status AS user_status,
                u.trust_score, u.store_verified, u.identity_verified,
    ...
    if (!verRes.rows.length) return fail(res, 404, "Store verification not found.");
    return res.json({ success: true, verification: { ...verRes.rows[0], timeline }, notes });
  } catch (err) {
...
router.post("/store/:id/approve", async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) { await client.query("ROLLBACK"); return fail(res, 404, "Store verification not found."); }
    const rec = rows[0];
    if (rec.current_status === "approved") { await client.query("ROLLBACK"); return res.json({ success: true, message: "Already approved." }); }
    if (!rec.identity_verified) { await client.query("ROLLBACK"); return fail(res, 422, "Cannot approve store — seller identity not verified."); }

    await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [rec.user_id]);
    await client.query(
      `UPDATE store_verifications SET status='approved', reviewed_by=$2, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
      [req.params.id, adminId]
    );
    await client.query(
      `UPDATE public.users SET store_verified=TRUE, store_verified_at=NOW(), updated_at=NOW() WHERE id=$1`,
      [rec.user_id]
    );
    const trustScore = await refreshAndPersistTrust(client, rec.user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "store", adminId, action: "approved", note });
    await client.query("COMMIT");

    notifyStoreApproved({ userId: rec.user_id, email: rec.email, name: rec.name });
    log({ adminId, action: "approve_store", targetId: req.params.id, details: `Approved store for user ${rec.user_id}`, meta: { trust_score: trustScore, note }, userId: rec.user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Store approved." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/store/:id/reject", async (req, res) => {
  ...
  try {
    ...
    await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [user_id]);
    await client.query(
      `UPDATE store_verifications SET status='rejected', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
      [req.params.id, reason, adminId]
    );
    await client.query(`UPDATE public.users SET store_verified=FALSE, updated_at=NOW() WHERE id=$1`, [user_id]);
    const trustScore = await refreshAndPersistTrust(client, user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "store", adminId, action: "rejected", note: reason });
    await client.query("COMMIT");

    notifyStoreRejected({ userId: user_id, email, name, reason });
    log({ adminId, action: "reject_store", targetId: req.params.id, details: `Rejected store for user ${user_id}`, meta: { reason, trust_score: trustScore }, userId: user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Store rejected." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/store/:id/reset", async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
      `SELECT sv.user_id, u.email, u.name FROM store_verifications sv
       JOIN public.users u ON u.id=sv.user_id WHERE sv.id=$1 FOR UPDATE OF sv`,
      [req.params.id]
    );
    if (!rows.length) { await client.query("ROLLBACK"); return fail(res, 404, "Store verification not found."); }
    const { user_id, email, name } = rows[0];
    ...
    await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [user_id]);
    await client.query(
      `UPDATE store_verifications SET status='reset', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
      [req.params.id, note, adminId]
    );
    await client.query(`UPDATE public.users SET store_verified=FALSE, updated_at=NOW() WHERE id=$1`, [user_id]);
    const trustScore = await refreshAndPersistTrust(client, user_id);
    await addNote(client, { verificationId: req.params.id, verificationType: "store", adminId, action: "reset", note });
    await client.query("COMMIT");

    notifyReset({ userId: user_id, email, name, note });
    log({ adminId, action: "reset_store", targetId: req.params.id, details: `Reset store for user ${user_id}`, meta: { note, trust_score: trustScore }, userId: user_id, ip });

    return res.json({ success: true, trust_score: trustScore, message: "Store reset." });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
...
router.post("/store/:id/assign", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE store_verifications SET assigned_admin_id=$2, assigned_at=NOW(), updated_at=NOW()
       WHERE id=$1 AND status='pending' RETURNING id, user_id`,
      [req.params.id, assignedAdminId]
    );
    if (!rows.length) return fail(res, 404, "Verification not found or not pending.");
    log({ adminId, action: "assign_store", targetId: req.params.id, details: `Assigned store to admin ${assignedAdminId}`, meta: { assigned_to: assignedAdminId }, userId: rows[0].user_id, ip });
    return res.json({ success: true, assigned_to: assignedAdminId });
  } catch (err) { return fail(res, 500, err.message); }
});
...
router.post("/store/:id/note", async (req, res) => {
  ...
  if (note.length > NOTE_MAX_LEN) return fail(res, 400, `Note must be ≤ ${NOTE_MAX_LEN} characters.`);
  try {
    const { rows } = await pool.query("SELECT id FROM store_verifications WHERE id=$1", [req.params.id]);
    if (!rows.length) return fail(res, 404, "Verification not found.");
    await addNote(pool, { verificationId: req.params.id, verificationType: "store", adminId, action: "note", note });
    return res.json({ success: true });
  } catch (err) { return fail(res, 500, err.message); }
});
...
router.post("/email/:userId/force-verify", requireSuperAdmin, async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
      "SELECT id, email, name FROM public.users WHERE id=$1 FOR UPDATE", [userId]
    );
    if (!rows.length) { await client.query("ROLLBACK"); return fail(res, 404, "User not found."); }
    const user = rows[0];
    await client.query(
      `UPDATE public.users SET email_verified=TRUE, email_verified_at=NOW(), verified=TRUE, updated_at=NOW() WHERE id=$1`,
      [userId]
    );
    const trustScore = await refreshAndPersistTrust(client, userId);
    await addNote(client, { verificationId: userId, verificationType: "email", adminId, action: "force_verified", note: "Email force-verified by super-admin." });
    await client.query("COMMIT");

    reactivateLimitedListings(userId).catch(() => {});
    createNotification({ userId, type: "email_verified", title: "Email Verified", message: "Your email has been verified by an administrator." }).catch(() => {});
    log({ adminId, action: "force_email_verify", targetId: userId, details: `Force-verified email for user ${userId}`, meta: { trust_score: trustScore }, userId, ip });
...
router.post("/email/:userId/revoke", requireSuperAdmin, async (req, res) => {
  ...
  try {
    await client.query("BEGIN");
    const { rows } = await client.query("SELECT id FROM public.users WHERE id=$1 FOR UPDATE", [userId]);
    if (!rows.length) { await client.query("ROLLBACK"); return fail(res, 404, "User not found."); }
    await client.query(
      `UPDATE public.users SET email_verified=FALSE, email_verified_at=NULL, verified=FALSE, updated_at=NOW() WHERE id=$1`,
      [userId]
    );
    await client.query(
    ...
    const trustScore = await refreshAndPersistTrust(client, userId);
    await addNote(client, { verificationId: userId, verificationType: "email", adminId, action: "revoked", note: reason });
    await client.query("COMMIT");

    createNotification({ userId, type: "email_revoked", title: "Email Verification Revoked", message: `Your email verification has been revoked. Reason: ${reason}` }).catch(() => {});
    log({ adminId, action: "revoke_email_verify", targetId: userId, details: `Revoked email for user ${userId}`, meta: { reason, trust_score: trustScore }, userId, ip });
...
router.post("/trust/:userId/recalculate", async (req, res) => {
  ...
  try {
    ...
    const trustScore = await refreshAndPersistTrust(client, userId);
    await client.query("COMMIT");
    log({ adminId, action: "recalculate_trust", targetId: userId, details: `Recalculated trust → ${trustScore}`, meta: { trust_score: trustScore }, userId, ip });
    return res.json({ success: true, trust_score: trustScore });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
    return fail(res, 500, err.message);
...
/* ══════════════════════════════════════════════════════════════
   9. UNIFIED  /:userId/approve|reject|reset
      Registered LAST — all static-prefix routes above match first.
══════════════════════════════════════════════════════════════ */
...
router.post("/:userId/approve", async (req, res) => {
  ...
  try {
    ...
    if (!userRows.length) {
      await client.query("ROLLBACK");
      console.warn("[approve] ✗ user not found:", userId);
      return fail(res, 404, `User not found (id: ${userId}).`);
    }
    ...
    if (user.status === "flagged" || user.status === "banned") {
      await client.query("ROLLBACK");
      return fail(res, 403, `Account is ${user.status} and cannot be approved.`);
    }

    if (!user.email_verified) {
      console.warn("[approve] ⚠ email not verified for:", userId);
    }

    /* ── Identity — any non-approved status ── */
    ...
    console.log("[approve] identity rows:", idRows.length,
      idRows[0] ? `status=${idRows[0].status}` : "none"
    );
    ...
    if (!idRows.length) {
      ...
      console.warn("[approve] ✗ no actionable identity — existing:", any);
      if (!any.length)
        return fail(res, 404,
    ...
    if (idRec.document_number_hash) {
      ...
      if (dup.length) {
        ...
        await addNote(client, {
          verificationId: idRec.id, verificationType: "identity", adminId,
          action: "flagged",
          note: `Blocked: document matches ${dup.length} approved account(s).`,
        });
        await client.query("COMMIT");
        console.warn("[approve] ✗ 409 duplicate doc count:", dup.length);
        log({ adminId, action: "flag_identity_duplicate", targetId: idRec.id, details: `Duplicate document for user ${userId}`, meta: { duplicate_count: dup.length }, userId, ip });
        return fail(res, 409,
          `Duplicate document detected on ${dup.length} other approved account(s). Flagged for manual review.`,
          { flagged: true }
        );
      }
    }
    ...
    console.log("[approve] store:", {
      hasStore, storeStatus: storeRec?.status ?? "none", storeNeedsUpdate,
    });
    ...
    await client.query(
      `UPDATE public.users
       SET identity_verified=TRUE, identity_verified_at=NOW(),
           store_verified    = CASE WHEN $2 THEN TRUE ELSE store_verified END,
           store_verified_at = CASE WHEN $2 THEN NOW() ELSE store_verified_at END,
           updated_at=NOW()
       WHERE id=$1`,
      [userId, hasStore]
    );
    ...
    /* ── Notes ── */
    const noteOps = [
      addNote(client, { verificationId: idRec.id, verificationType: "identity", adminId, action: "approved", note }),
    ];
    if (storeNeedsUpdate)
      noteOps.push(addNote(client, { verificationId: storeRec.id, verificationType: "store", adminId, action: "approved", note }));
    await Promise.all(noteOps);

    await client.query("COMMIT");

    console.log("[approve] ✓ done | trust:", trustScore,
      "| idWas:", idRec.status,
      "| storeWas:", storeRec?.status ?? "none"
    );

    reactivateLimitedListings(userId).catch((e) =>
      console.error("[approve] reactivate err:", e.message)
    );
    notifyApproved({ userId, email: user.email, name: user.name });

    log({
      adminId, action: "approve_all", targetId: userId,
      details: `Approved identity${hasStore ? " + store" : ""} for user ${userId}`,
      meta: {
        ...
        has_store: hasStore, trust_score: trustScore, note,
      },
      userId, ip,
    });
    ...
    return res.json({
      ...
      approved   : { identity: idRec.id, store: storeRec?.id ?? null },
      message    :
...
router.post("/:userId/reject", async (req, res) => {
  ...
  try {
    ...
    const { rows: userRows } = await client.query(
      `SELECT id, email, name, status FROM public.users WHERE id=$1 FOR UPDATE`,
      [userId]
    );
    if (!userRows.length) { await client.query("ROLLBACK"); return fail(res, 404, "User not found."); }
    const user = userRows[0];

    const { rows: idRows } = await client.query(
      `SELECT id, status FROM identity_verifications
       WHERE user_id=$1 AND status IN ('pending','flagged')
       ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
      [userId]
    );
    const { rows: storeRows } = await client.query(
      `SELECT id, status FROM store_verifications
       WHERE user_id=$1 AND status IN ('pending','reset')
       ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
      [userId]
    );
    ...
    if (idRows.length) {
      await client.query(
        `UPDATE identity_verifications SET status='rejected', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
        [idRows[0].id, reason, adminId]
      );
      await addNote(client, { verificationId: idRows[0].id, verificationType: "identity", adminId, action: "rejected", note: reason });
    }
    if (storeRows.length) {
      await client.query(
        `UPDATE store_verifications SET status='rejected', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
      ...
      await addNote(client, { verificationId: storeRows[0].id, verificationType: "store", adminId, action: "rejected", note: reason });
    }

    await client.query(
      `UPDATE public.users SET identity_verified=FALSE, store_verified=FALSE, updated_at=NOW() WHERE id=$1`,
      [userId]
    );
    ...
    console.log("[reject] ✓ userId:", userId, "trust:", trustScore);
    notifyRejected({ userId, email: user.email, name: user.name, reason });
    log({
      adminId, action: "reject_all", targetId: userId,
      details: `Rejected identity+store for user ${userId}`,
      meta: { identity_verification_id: idRows[0]?.id??null, store_verification_id: storeRows[0]?.id??null, reason, trust_score: trustScore },
      userId, ip,
    });
    ...
    return res.json({
      success    : true,
      trust_score: trustScore,
      rejected   : { identity: idRows[0]?.id??null, store: storeRows[0]?.id??null },
...
router.post("/:userId/reset", async (req, res) => {
  ...
  try {
    ...
    const { rows: idRows } = await client.query(
      `SELECT id FROM identity_verifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
      [userId]
    );
    const { rows: storeRows } = await client.query(
      `SELECT id FROM store_verifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
      [userId]
    );
    ...
    if (idRows.length) {
      await client.query(
        `UPDATE identity_verifications SET status='reset', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
        [idRows[0].id, note, adminId]
      );
      await addNote(client, { verificationId: idRows[0].id, verificationType: "identity", adminId, action: "reset", note });
    }
    if (storeRows.length) {
      await client.query(
        `UPDATE store_verifications SET status='reset', rejection_reason=$2, reviewed_by=$3, reviewed_at=NOW(), updated_at=NOW() WHERE id=$1`,
      ...
      await addNote(client, { verificationId: storeRows[0].id, verificationType: "store", adminId, action: "reset", note });
    }
    ...
    console.log("[reset] ✓ userId:", userId, "trust:", trustScore);
    notifyReset({ userId, email: user.email, name: user.name, note });
    log({
      adminId, action: "reset_all", targetId: userId,
      details: `Reset identity+store for user ${userId}`,
      meta: { identity_verification_id: idRows[0]?.id??null, store_verification_id: storeRows[0]?.id??null, note, trust_score: trustScore },
      userId, ip,
    });
    ...
    return res.json({
      success    : true,
      trust_score: trustScore,
      reset      : { identity: idRows[0]?.id??null, store: storeRows[0]?.id??null },

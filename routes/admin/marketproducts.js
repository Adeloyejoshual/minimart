...
router.get("/:id", async (req, res) => {
  try {
    ...
    const [images, variants, features, specs, boxItems] = await Promise.all([
      pool.query(`SELECT image_url, public_id, is_primary, sort_order
                  FROM market.product_images WHERE product_id = $1 ORDER BY sort_order`, [p.id]),
      pool.query(`SELECT id, sku, name, price, stock, attributes
                  FROM market.product_variants WHERE product_id = $1 ORDER BY created_at`, [p.id]),
      pool.query(`SELECT feature FROM market.product_features
                  WHERE product_id = $1 ORDER BY sort_order`, [p.id]),
      pool.query(`SELECT spec_key, spec_value FROM market.product_specifications
                  WHERE product_id = $1 ORDER BY sort_order`, [p.id]),
      pool.query(`SELECT item FROM market.product_box_items
                  WHERE product_id = $1 ORDER BY sort_order`, [p.id]),
    ]);
...
router.post("/:id/approve", async (req, res) => {
  try {
    ...
    await log(req.admin.id, "approve_market_product", req.params.id, `Approved market product ${req.params.id}`);
...
router.post("/:id/reject", async (req, res) => {
  ...
  try {
    ...
    await log(req.admin.id, "reject_market_product", req.params.id,
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    await log(req.admin.id, "edit_market_product", req.params.id,
      `Edited market product "${rows[0].name}" — ${changedFields}`, req.body);

    res.json({ success: true, message: "Market product updated" });
  } catch (err) {
...
router.post("/:id/flag", async (req, res) => {
  ...
  try {
    ...
    if (!rowCount) return res.status(404).json({ error: "Market product not found" });
    await log(req.admin.id, value ? `set_${flag}` : `unset_${flag}`, req.params.id, `${flag} set to ${value}`);
...
router.post("/:id/remove", async (req, res) => {
  ...
  try {
    ...
    await log(req.admin.id, "remove_market_product", req.params.id,
      `Removed market "${rows[0].name}" — reason: ${reason.trim()}`);

    res.json({ success: true, message: "Market product removed" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete("/:id/permanent", requireSuperAdmin, async (req, res) => {
  try {
    ...
    await log(req.admin.id, "permanent_delete_market", req.params.id,
      `Permanently deleted market "${rows[0].name}"`);

    res.json({ success: true, message: "Market product permanently deleted" });
  } catch (err) {

...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    /* Manual sort override — always keeps promoted products first */
    switch (sort) {
      case "price_asc":
    ...
    /* Count only on page 0 — expensive on large tables */
    ...
    const featured = (Number(page) === 0 && !section)
      ? products.filter((p) => p.is_promoted && p.promotion_priority >= 3).slice(0, 6)
      : [];
